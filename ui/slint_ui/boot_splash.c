...
#define SPLASH_RGB565(r, g, b) ((uint16_t)((((r) >> 3) << 11) | (((g) >> 2) << 5) | ((b) >> 3)))
...
static uint16_t splash_blend(uint16_t dst, uint16_t src, unsigned covered, unsigned samples)
{
  ...
  const unsigned red = ((((src >> 11) & 0x1fu) * covered) + (((dst >> 11) & 0x1fu) * inverse) + half) / samples;
  const unsigned green = ((((src >> 5) & 0x3fu) * covered) + (((dst >> 5) & 0x3fu) * inverse) + half) / samples;
  const unsigned blue = (((src & 0x1fu) * covered) + ((dst & 0x1fu) * inverse) + half) / samples;
...
static void splash_fill_rect(const splash_canvas_t *canvas, int x, int y, int w, int h, uint16_t color)
{
  if (w <= 0 || h <= 0) {
    return;
  }
...
static void splash_fill_shape(const splash_canvas_t *canvas, splash_shape_t shape, uint16_t color)
{
  ...
  for (int py = top; py < bottom; ++py) {
    ...
    for (int px = left; px < right; ++px) {
      ...
      *pixel = covered == SPLASH_AA_SAMPLES ? color : splash_blend(*pixel, color, covered, SPLASH_AA_SAMPLES);
    }
  }
}
...
{
  splash_fill_rect(canvas, canvas->origin_x, canvas->origin_y, canvas->width, canvas->height, canvas->base_color);
}
...
static void splash_paint_dots(const splash_canvas_t *canvas, int highlight)
{
  ...
  for (int index = 0; index < SPLASH_DOT_COUNT; ++index) {
    const splash_shape_t dot = splash_circle_shape(SPLASH_DOTS_CX + offsets[index][0],
                                                   SPLASH_CONTROLS_CY + offsets[index][1], SPLASH_DOT_RADIUS);
    splash_fill_shape(canvas, dot, index == highlight ? SPLASH_COLOR_TERTIARY : SPLASH_COLOR_OUTLINE);
  }
}
...
static void splash_paint_frame(const splash_canvas_t *canvas, int highlight, int filled, uint16_t fill_color)
{
  splash_clear(canvas);
  ...
  splash_fill_rect(canvas, SPLASH_DPAD_CX - SPLASH_DPAD_HALF, SPLASH_CONTROLS_CY - SPLASH_DPAD_THICK / 2,
                   SPLASH_DPAD_HALF * 2, SPLASH_DPAD_THICK, SPLASH_COLOR_PRIMARY);
  splash_fill_rect(canvas, SPLASH_DPAD_CX - SPLASH_DPAD_THICK / 2, SPLASH_CONTROLS_CY - SPLASH_DPAD_HALF,
                   SPLASH_DPAD_THICK, SPLASH_DPAD_HALF * 2, SPLASH_COLOR_PRIMARY);
...
{
  if (dots_changed) {
    esp_err_t result = panel_transfer(s_splash.dots, SPLASH_DOTS_X, SPLASH_DOTS_Y, SPLASH_DOTS_W, SPLASH_DOTS_H);
    if (result != ESP_OK) {
      ESP_LOGW(TAG, "dots transfer failed: %s", esp_err_to_name(result));
    }
  }
  esp_err_t result = panel_transfer(s_splash.bar, SPLASH_TRACK_X, SPLASH_TRACK_Y, SPLASH_TRACK_W, SPLASH_TRACK_H);
  if (result != ESP_OK) {
...
static uint16_t *splash_alloc(size_t pixels, const char *what)
{
  ...
  uint16_t *buffer = heap_caps_aligned_alloc(64, bytes, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT | MALLOC_CAP_DMA);
  if (buffer != NULL) {
  ...
  if (buffer != NULL) {
    ESP_LOGW(TAG, "splash %s: %u bytes fell back to PSRAM (internal DMA unavailable)", what, (unsigned)bytes);
  }
  return buffer;
}
...
static void splash_anim_step(void)
{
  ...
  const int highlight = (int)((uint64_t)SPLASH_DOT_COUNT * covered_ms / s_splash.total_ms) % SPLASH_DOT_COUNT;
...
{
  if (s_splash.active || stage_ms == NULL || stage_count == 0 || stage_count > SPLASH_STAGE_MAX) {
  ...
  if (frame == NULL) {
    ESP_LOGE(TAG, "splash frame buffer unavailable (%u bytes)", (unsigned)(frame_pixels * sizeof(uint16_t)));
  ...
  ESP_LOGI(TAG, "splash painted (%dx%d), backlight %u%%", SPLASH_WIDTH, SPLASH_HEIGHT, (unsigned)brightness_pct);
  return ESP_OK;
}
