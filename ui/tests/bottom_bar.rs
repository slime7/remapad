...
fn 底栏三格按整屏均分且图标与标签同轴居中() {
    ...
    let cells = ["BottomBar::mode-cell", "BottomBar::host-cell", "BottomBar::battery-cell"];
    let icons = ["BottomBar::mode-icon", "BottomBar::host-icon", "BottomBar::battery-icon"];
    let labels = ["BottomBar::mode-text", "BottomBar::host-leds", "BottomBar::battery-text"];
    for (index, id) in cells.iter().enumerate() {
        let cell = ui::rect(&app, id);
        let center = CELL_CENTERS[index];
        assert!((cell.x - (center - 40.0)).abs() < 0.5 && (cell.w - 80.0).abs() < 0.5, "{id} 不是 80 宽的等分格：{cell:?}");
        assert!((cell.center_x() - center).abs() < 0.5, "{id} 的格心 {:.1} 不在 {center}", cell.center_x());
    ...
    for index in 0..cells.len() {
        ...
        assert!((icon.0 - center).abs() < 1.5, "{} 的图标墨迹中线 {:.1} 不在格心 {center}", icons[index], icon.0);
        assert!((icon.1 - ICON_CENTER_Y).abs() < 1.5, "{} 的图标没落在这条中线上", icons[index]);
        assert!((label.0 - center).abs() < 1.5, "{} 的标签墨迹中线 {:.1} 不在格心 {center}", labels[index], label.0);
        assert!((label.1 - LABEL_CENTER_Y).abs() < 1.5, "{} 的标签没落在这条中线上", labels[index]);
    }
}
...
fn 手柄操控提示行的图标与文字同高() {
    ...
    for part in parts {
        ...
        let size = part.size();
        let part_box = Rect { x: position.x, y: position.y, w: size.width, h: size.height };
    ...
    for (_, row) in &rows {
        let raised: Vec<f32> =
            row.iter().map(|p| ui::ink_center_y(&frame, *p).expect("行里的元素没画出来")).collect();
        let spread = raised.iter().cloned().fold(f32::MIN, f32::max)
            - raised.iter().cloned().fold(f32::MAX, f32::min);
        ...
        let span = Rect { x: left, y: row[0].y, w: right - left, h: 17.0 };
...
fn ota进度条居中且从条槽左端起填充() {
    ...
    for percent in [0, 25, 50, 100] {
        ...
        assert!((fill.x - track.x).abs() < 0.5, "{percent}% 的进度没从条槽左端起：{fill:?}");
        assert!((fill.w - expected).abs() < 0.5, "{percent}% 的进度宽 {:.1} 不是 {expected}", fill.w);
    }

    app.set_ota_phase(0);
    assert!(
        ui::ElementHandle::find_by_element_id(&app, "BottomBar::ota-track").next().is_none(),
        "非升级状态还画着进度条"
    );
}
...
fn band(center_x: f32, y: f32, height: f32) -> Rect {
    Rect { x: center_x - 22.0, y, w: 44.0, h: height }
}
