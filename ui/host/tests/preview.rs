...
fn 预览控制条翻页后设备画面切到下一张卡片() {
  ...
  assert!(
    track.bottom() <= DEVICE_HEIGHT as f32,
    "亮度页没画在设备视图里：{track:?}"
  );
...
fn 预览里按确认键走设备上的焦点分发() {
  ...
  assert!(
    ui::element_or_none(&preview, "ConfirmDialog::confirm-btn").is_some(),
    "确认弹窗没画出来"
  );
  ...
  assert!(
    ui::element_or_none(&preview, "ConfirmDialog::confirm-btn").is_none(),
    "弹窗没有关"
  );
}
