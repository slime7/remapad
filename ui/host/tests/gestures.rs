...
fn 拖动时条带跟手到切页阈值就定住() {
  ...
  assert!(
    (follow - (BAND_REST_X + 20.0)).abs() < 1.0,
    "位移 20px 时条带应跟到 20px：{follow:.1}"
  );
...
fn 拖动越过阈值抬手翻到上一页() {
  ...
  assert!(
    (rest - BAND_REST_X).abs() < 0.5,
    "滑行结束后条带应回到静止位置：{rest:.1}"
  );
}
...
fn 滑行期间按住拖动不再跟手() {
  ...
  let held = ui::rect(&preview, "AppContent::band").x;
  assert!(
    (held - sliding).abs() < 0.5,
    "滑行期间条带不该再跟手：{held:.1} 对 {sliding:.1}"
  );
}
