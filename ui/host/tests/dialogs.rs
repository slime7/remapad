...
fn 弹窗打开时遮罩盖住整屏() {
    ...
    assert!((box_rect.center_x() - center).abs() < 0.5, "弹窗盒没横向居中：{box_rect:?}");
    assert!(ui::element_or_none(&app, "ConfirmDialog::cancel-btn").is_some(), "弹窗按钮没画出来");
    assert_eq!(covered.count_color(plus, CORNER_BG, 8), 0, "遮罩下的页面还留着原来的底色");
    let (dim_before, dim_after) = (mean_luma(&before), mean_luma(&covered));
    assert!(dim_after * 1.2 < dim_before, "遮罩没把整屏压暗：{dim_before:.1} → {dim_after:.1}");
}
...
fn 弹窗打开时页面控件不再响应点按() {
    ...
    ui::frame(&preview);
    ui::tap(&preview, plus.center_x(), plus.center_y());
    assert_eq!(preview.get_backlight(), start + 20, "弹窗盖住页面后同一处点按不该改背光");
}
...
fn 重启与关机等待画面盖住整屏() {
    ...
    ui::settle(&app);
    let plus = ui::rect(&app, "BrightnessPage::plus");
    let full = ui::Rect { x: 0.0, y: 0.0, w: ui::SCREEN_WIDTH as f32, h: ui::SCREEN_HEIGHT as f32 };
    let middle = ui::Rect { x: 0.0, y: 120.0, w: ui::SCREEN_WIDTH as f32, h: 40.0 };
    ...
    for (what, set) in cases {
        ...
        assert!(normal.count_color(plus, CORNER_BG, 8) > 0, "{what}画面收掉后页面应照常画出来");
    }
}
