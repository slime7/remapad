...
const VIEW: Viewport = Viewport { width: 240, height: 280 };
...
fn 整屏重画按四十八行切分且末段取余数() {
    let full = Damage { x: 0, y: 0, width: VIEW.width as u32, height: VIEW.height as u32 };
...
fn 不满宽的矩形按自己的宽度逐行取像素() {
    ...
    let damage = Damage { x: 60, y: 100, width: 40, height: 60 };
    ...
    assert_eq!(submitted[0], Band { x: 60, y: 100, width: 40, rows: 48 });
    assert_eq!(submitted[1], Band { x: 60, y: 148, width: 40, rows: 12 }, "末段的行号与余数不对");
}
...
fn 越过视口右下角的矩形裁到视口内() {
    ...
    let plan: Vec<Band> = bands(
        Damage { x: 200, y: 270, width: 80, height: 40 },
        VIEW,
        BAND_ROWS,
    )
    .collect();
    assert_eq!(plan, vec![Band { x: 200, y: 270, width: 40, rows: 10 }], "越界部分应裁掉");
...
fn 负坐标的矩形裁到视口内() {
    let frame = synthetic(VIEW);
    let plan: Vec<Band> = bands(Damage { x: -30, y: -10, width: 60, height: 30 }, VIEW, BAND_ROWS).collect();
    assert_eq!(plan, vec![Band { x: 0, y: 0, width: 30, rows: 20 }], "越出左上角的部分应裁掉");
...
fn 空的或完全在视口外的矩形不产生行带() {
    let cases = [
        Damage { x: 10, y: 10, width: 0, height: 40 },
        Damage { x: 10, y: 10, width: 40, height: 0 },
        Damage { x: 240, y: 0, width: 40, height: 40 },
        Damage { x: -50, y: 10, width: 50, height: 40 },
        Damage { x: 0, y: 280, width: 40, height: 40 },
    ];
    for case in cases {
...
#[test]
fn 行带高度为零时按一行切分() {
    let plan: Vec<Band> = bands(Damage { x: 0, y: 0, width: 240, height: 3 }, VIEW, 0).collect();
