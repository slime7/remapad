...
fn 调试页画在末位槽号上且只画当前页() {
  ...
  for (id, button) in buttons.iter().zip(boxes) {
    ...
    assert!(
      filled * 2 > (button.w * button.h) as usize,
      "{id} 没画出来：底色只占 {filled}"
    );
  }
...
fn 切页时整条带划过一整页再回到静止位置() {
  ...
  assert!(
    (settled - rest).abs() < 0.5,
    "滑行结束后条带没回到静止位置：{settled:.1}"
  );
...
fn usb模式页两卡的图标对齐在卡片内边距上() {
  ...
  for id in cards {
    ...
    let icons = ui::element(&app, id)
      .query_descendants()
      .match_type_name("IconBox")
      .find_all();
  ...
  assert!(
    (icon_x[0] - icon_x[1]).abs() < 0.5,
    "两卡的图标没落在同一条竖线上：{icon_x:?}"
  );
}
