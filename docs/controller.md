...
> **复合输出通道 `0x0016` 的命令帧带 33 字节 `0x00` 前缀**：Switch 1 的「0x00 + 2×16B 震动 + 命令」布局未被观测到。
...
- 进入游戏后主机会以接近输入上报的频率（约 66 Hz）**持续刷零幅度的保活包**——状态字使能位（bit6）为 1、三组振幅全 0，实体手柄收到同样不震。
  「在震」判据因此是使能位**且**归一强度高过载波电平（>2/255）；
...
- 映射设备按布局行逐马达跟带（重击马达跟低频、纹理马达跟高频）；主机振幅是 LRA 线性档位，
  ERM 偏心马达（DualSense / DualShock / Xbox）低占空比整段落在死区，
  归一时按感知曲线重映射（非零档落地不低于 53/255、满幅不变）再供马达编码与合成通路消费。
...
| `0x15` | 读取 NFC 数据缓冲区 | 2B 缓冲区读取偏移 (小端序) | 状态 + 数据长度 (u16 LE) + 缓冲数据 (最大 70 字节) | 从手柄分块提取已读取的 amiibo 镜像；`00 46 00` 的 `46 00` 是**数据长度**而非偏移回显（偏移 70 与块长 70 巧合相同，2026-09-19 实机对账钉死：主机按长度前进指针、拉到长度 0 结束） |
...
- **NFC 状态字节（2026-09-19 实机对账验证到抽块完成）**：输入报告 0x09 的状态字节与 0x05 体首字节**同源**，
...
手柄直插板卡时由板上现场合成 PCM（`usb/usb_audio.c`：两带各一个相位连续的正弦，频率取参数包的 9 位频率字段、为 0 或越界回落缺省值，
