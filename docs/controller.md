...
- **本设备的音色表实现**（`pad/feedback.c`）：按采样 ID 登记各自的幅度时间线（段边界毫秒 → 段内幅度，新增采样只加数据行），
  登记条目按各自周期循环、未登记的回落一次短脉冲；数据面按音色段步进，
  把渲染幅度替换进编码与写回用的采样字节（HID 马达、板上合成、PC 合成三处消费同一幅度）。
...
插在 PC 上时音频接口被 Windows 持有，由 PC 侧对同一端点开 WASAPI 流做哑渲染，参数（两带振幅、频率落地值、采样幅度）经桥接 FEEDBACK 帧下发、落地规则只在固件一份。
