...
- 主机向 `0x0012` 写入震动参数包，两种形态都观测过：**42 字节**（`0x00` 占位字节 + 左右两条
  ...
  接收侧按长度自适应（≥33 字节剥 1 字节前缀，见 `ns2_rumble_parse`）；把 42 字节判成
  33 字节丢包或把 32 字节判成多一包都会错位。
- 进入游戏后主机会以接近输入上报的频率（约 66 Hz）**持续刷零幅度的保活包**——状态字使能位（bit6）为 1、三个子帧振幅全 0
  （游戏内采集中超过一半的包是保活包），实体手柄收到同样不震。状态字 bit4-5 声明有效子帧数
  （载波包为 1、游戏包多为 1-3）。
  「查找手柄」页的参数包则带静止包形态（高频频率 `0x1E1`、零振幅，维持 LRA 通路；
  蜂鸣本体走 0x0A 采样流）。「在震」判据因此是使能位**且**归一强度高过载波电平（>2/255）；
  `0x0012` 的接收回调不得做阻塞 IO，游戏内的持续震动流会拖住 BLE 主机任务、输入通知随之停摆。「查找手柄」页的参数包则带静止包形态（高频频率 `0x1E1`、零振幅，维持 LRA 通路；
  蜂鸣本体走 0x0A 采样流）。「在震」判据因此是使能位**且**归一强度高过载波电平（>2/255）；
...
- 映射设备按布局行消费主机波形：声明了 HD 触觉通路的设备（DualSense）在布局行的 `hd` 规则里把三个时序子帧**按时间顺序重整**成
  触觉音圈的 PCM（震动映射为震动、采样发声段映射为扬声器并折进音圈，见 [ADR 0046](adr/0046-ns-waveform-to-ds5-pcm-hd-haptics.md)）；
...
手柄直插板卡时由板上现场合成 PCM（`usb/usb_audio.c`：子帧按时间顺序轮播、每带一个相位连续的正弦，频率码按 9 位 log2 刻度解成 Hz、为 0 或越界回落缺省值，
