...
- **广播地址必须是 public 类型**：派生静态随机地址的广播（厂商数据逐字节符合上表）在主机「更改 Grip/顺序」界面完全不可见，也不发起连接；
  ...
  模拟实现用固定伪装 OUI（本设备 `9C:E6:35`）只为让主机把它当新设备重配一次。
...
- **运动数据长度必须非零**：主机开启 IMU 特性位（0x0C 掩码含 bit2）后，`0x0E` 为 0 的报文同样不被采用。
  ...
  本设备另有 `motion 3` 实验档按 NS1 的三份 12 字节样本风格填块首（默认关闭，只用于实机 A/B），
  真 Switch 2 手柄插在板卡上时走同代透传、运动块原样到达主机（[ADR 0026](adr/0026-same-generation-input-passthrough.md)）。
...
位串按 SDL 的 HD 震动编码解释（SDL_hidapi_switch2.c `EncodeHDRumble`）：BlueRetro sw2.h
对同一位串给出了带序相反、含音色使能位的另一套解释——振幅的归一值在两种解释下一致，
本节取 SDL 的形态。NS 的震动是**波形描述**而不是马达信号：状态字声明 n 个有效子帧时，
...
| 位 | 字段 | 说明 |
| :--- | :--- | :--- |
| bit 0-9 | HF Freq | 高频频率（10 位） |
| bit 10-19 | HF Amp | 高频振幅（10 位） |
| bit 20-29 | LF Freq | 低频频率（10 位） |
| bit 30-39 | LF Amp | 低频振幅（10 位） |

BlueRetro 的静止包常量 `0x1E100000`（= 低频频率 `0x1E1`、零振幅）钉住了低频频率的位位置；
实机抓包样例 `04 80 01 97 63` 按此解出高频 4 / 96、低频 368 / 398。

**BLE 写入形态与主机震动流行为（实机对账）**：

- BLE 模式下主机向 `0x0012` 写入的是**不带 Report ID 的 32 字节**（左 16B + 右 16B 参数包）；
  上表的 42 字节是 USB 形态（Report ID + 双包 + 9B 保留）。
  把 BLE 长度判成 33 字节会把每一包震动都丢掉，并每 20-30 ms 刷一条告警。
- 进入游戏后主机会以接近输入上报的频率（约 66 Hz）**持续刷零幅度的保活包**——状态字使能位（bit6）为 1、三个子帧振幅全 0，
  实体手柄收到同样不震。「查找手柄」页的参数包则带静止包形态（低频频率 0x1E1、零振幅，维持 LRA 通路；
  蜂鸣本体走 0x0A 采样流）。「在震」判据因此是使能位**且**归一强度高过载波电平（>2/255）；
  `0x0012` 的接收回调不得做阻塞 IO，游戏内的持续震动流会拖住 BLE 主机任务、输入通知随之停摆。
- 震动流本质是音频式包络（低频给冲击、高频给纹理），参数包逐包都在抖（低有效位、频率扫描）而马达字节常常不变：写回侧按「编码后的报告字节变化才发」去重，拿原始字节当变化判据会把等价帧以接近输入上报的频率照发不误。
- 映射设备按布局行消费主机波形：声明了 HD 触觉通路的设备（DualSense）在布局行的 `hd` 规则里把三个时序子帧**按时间顺序重整**成
  触觉音圈的 PCM（震动映射为震动、采样发声段映射为扬声器，见 [ADR 0046](adr/0046-ns-waveform-to-ds5-pcm-hd-haptics.md)）；
  没有音频触觉通路的设备按布局行逐马达跟带（重击马达跟低频、纹理马达跟高频）。主机振幅是 LRA 线性档位，
  ERM 偏心马达（DualSense / DualShock / Xbox）低占空比整段落在死区，
  归一时按感知曲线重映射（非零档落地不低于 53/255、满幅不变）再供马达编码消费（HD 的 PCM 波形走原始档位线性直迁）。
...
- **本设备的音色表实现**（`pad/feedback.c`）：采样是主机点播的声音（真手柄用 HD 马达放声），本设备不把它转成震动——
  音色表按采样 ID 登记各自的响/停时间线（段边界毫秒 → 段内幅度，新增采样只加数据行），
  登记条目按各自周期循环、未登记的回落一次短脉冲；输入设备有线接入时（USB host 直插或桥接转发的有线手柄）
  数据面按音色段驱动板载蜂鸣器发声（鸣叫时长取段剩余），蓝牙手柄直接丢弃：
  马达编码、板上音圈合成与桥接 FEEDBACK 合成都不消费采样。
  0x0A 采样是运行期热路径，命令留痕按秒聚合。
...
**模拟侧的「重启即重推」循环坑**：真机在 `0x0d/0x07` 后重启进新固件，主机真正等的是「控制器断开后带着新版本回来」——
模拟设备若跟着重启，主机会当更新没生效而重推整包（推包 → 重启 → 再推包循环）。
设备因此默认不重启，逐帧回空体让主机走完流程；把上报版本抬到足够高后主机不再主动弹「手柄有更新」，是否根治仍未确认（主机认的版本号未知）。
记录流装配在 `firmware/main/target/ns2/ns2_upgrade.c`（主机端用例用抓包字节钉住），现场旋钮见串口 `fwver` / `fwack` / `fwpost` / `fwapply`。
...
本设备四段都可配置（手柄设置页给预设款式、串口 `ctrl` 手工指定），未配置时取 Pro Controller 2 的抓包值；
配色在主机连接时读取（0x13000 出厂块），改完配色打开连接窗口让主机重连一次即生效。
...
板卡没有 NFC 前端，模拟读卡器要额外硬件；本设备把「标签」整个搬进软件——预置一份 NTAG215 镜像
（amiibo dump，540 字节），主机的 Command 0x01 通路按上面表格逐一应答，主机看到的就是一张常驻感应区的卡：

- **镜像从哪来**：PC 侧（`remapadctl.py --amiibo` / 交互模式 `:amiibo`，将来的图形界面同一通道）经桥接帧
  `0x40-0x43` 上传（接受 540 纯镜像或 572 = 镜像 + 尾部 32B 厂商签名两种尺寸），落 storage 分区 SPIFFS
  槽位（`amiibo/amiibo_store.c`，200 槽，572B 记录，名称取文件名主干）；
  串口 `amiibo select <n>` 选中（持久化、重启恢复），`amiibo list / del / poll / hdr` 管理与验证。
...
- **读卡缓冲区布局**：`[60 字节读卡结果头][540 字节标签镜像]` 共 600 字节——抓包里偏移 70 的应答数据
  ...
  PC 端用泄露密钥（amiitool 算法 + key_retail）验证手头 73 份 dump：72 份双 HMAC 全部有效，
  镜像数据不是卡点；全零头是「抽完静默重来」的头号嫌疑。`amiibo hdr 0|1` 运行时可切全零头/结构头对账。
- **已验证的传输行为（实机）**：主机开轮询 → 0x05 拿到 UID → 0x06 触发 → 等 0x15 →
  按 70 字节连抽缓冲（严格跟随应答的长度字段前进指针）→ 拉到长度 0 探测结束。整份传输每轮都能干净完成
  （全零头与结构头两种形态同样干净），但主机抽完仍静默约 3 秒后整轮重来，游戏（马力欧惊奇）全程无提示无报错。
- **已排除的收尾假说（实机扫值）**：抽块结束（EOF 探测）后把报告状态字节与 0x05 体首字节
  逐值设为 0x00-0x17（两种头形态各一轮）均不改变结局；抽完后主动补发一帧应答
  （0x05 带 `31 04` 标记、0x05 纯卡信息、0x06 空体、0x15 空体四种）同样无效。
- **外部调研**：公开渠道没有任何「NS2 虚拟手柄跑通 amiibo」的先例——GitHub 仓库搜索
  `switch2 controller nfc` / `switch 2 amiibo emulator` 均零结果，现有 NS2 手柄模拟项目
  （zhantss/ESP32-BLE5-NSController-Emulator、TommyWabg/Switch2Connect、BlueRetro、Flipper 应用）无一实现 NFC；
  NS2 上能用的 amiibo 方案全是标签侧（Flipper/实体卡/手机模拟 NTAG215，由真手柄的读卡器读）
  或主机侧 homebrew（emuiibo）。Switch 1 先例是 joycontrol 在输入报告里推送标签数据
  （`agent-temp/joycontrol_mcu.py`，推送式机制与 NS2 拉取式协议不同，不可移植）。
  最后一公里（抽块完成后主机的收尾期待）只能靠真手柄读真卡的抓包（BLE 抓包器 + LTK，或 USB 抓包）钉死，
  暂缺抓包与 amiibo 资产，工作暂停。
...
- **蓝牙解密抓包补证（JoyCon 2）**：0x0C 的完整往返 = 主机往 0x0016 复合写 `01 91 01 0C`（前导零 = 该手柄输出报告
  ...
  `01 01 01 0C 10 78 00 00 61 12 50 0D`；真机应答通知不带 14 字节前缀，而本设备带前缀的形态已被主机接受，维持不变。
...
- **实现位置**：状态机与 Command 0x01 分发在 `firmware/main/target/ns2/ns2_nfc.c`（主机端用例
  `firmware/test/test_ns2_nfc.c` 用抓包黄金字节钉住），上传会话在 `firmware/main/amiibo/`（用例 `test_amiibo_proto.c`），
  ble_session 的 `case 0x01` 只做转接。整条读取流程仍需真机互操作验证（应答体按抓包实现、
  尚未在真实主机上跑通过一次 amiibo 读取）。
...
设备上电静默；连接键（配对页「连接」按钮与 PWR 长按 3 秒）打开 30 秒连接窗口——已配对身份前 3 秒发 0x81 唤醒突发叫醒休眠主机、随后转 0x00 回连形态等醒着的主机连回来，未配对身份进配对流程发发现广播；
主机主动断开（休眠或移开）后自动开 30 秒回连窗口，全程只发 0x00 回连形态、不带唤醒突发——链路断开可能正是用户主动休眠主机，回连不得把它立刻叫起来，30 秒未连上则彻底关闭广播进入静默（用户主动断开的场景不开窗，保持静默）；
...
**配对状态的注册判据**：主机的配对记录按控制器地址存（见「广播过滤与配对记录」），本设备地址固定不变，对同一台主机按「配对 新主机」不会重走 0x15；
...
判据为纯函数 `ns2_adv_host_registered()`，主机端用例钉住「已订阅但没启用特性」不算注册。
...
**多路输出的可行边界**：基础设施允许两条连接（`CONFIG_BT_NIMBLE_MAX_CONNECTIONS=2`）
与两个广播实例（`CONFIG_BT_NIMBLE_MAX_EXT_ADV_INSTANCES + 1`，当前配置 1 即实例 0/1 可用），控制器活动预算 `CONFIG_BT_CTRL_BLE_MAX_ACT=6` 也够用；
**唯一的硬限制是地址类型**——NimBLE 每个广播实例的地址只接受 RANDOM（`ble_gap_ext_adv_set_addr` 要求 `addr->type == BLE_ADDR_RANDOM`），
...
节奏因此写死在数据面任务（`DP_SEND_DIV`），不提供运行时档位。
...
### 反馈的下游投递与 DS5 音频触觉（本设备实现）

主机反馈（震动 / 玩家灯 / 触觉采样）经 `pad/feedback.c` 按输入设备的布局行编码回写；DualSense 另有一条音频触觉真通路——
它的 USB 音频接口是 UAC1 48kHz / 16bit / 4ch PCM（实机描述符：音频流接口只有一组 alt，等时 OUT 端点 MPS 392、单一采样率；
AC 输入端子通道配置 `0x33` = FL+FR+RL+RR，后两路直连左右触觉音圈，PS5 同款用法）。
手柄直插板卡时由板上现场合成 PCM（`usb/usb_audio.c`：子帧按时间顺序轮播、每带一个相位连续的正弦，频率取参数包的 10 位频率字段、为 0 或越界回落缺省值，
每毫秒一包 384 字节、3 个在途传输）；
插在 PC 上时音频接口被 Windows 持有，由 PC 侧对同一端点开 WASAPI 流做哑渲染，参数（两带振幅、频率落地值）经桥接 FEEDBACK 帧下发、落地规则只在固件一份；
触觉采样不进任何渲染通路——输入设备有线接入时由板载蜂鸣器按音色表发声、蓝牙手柄直接丢弃（见「Command 0x0A」）。
音频接手期间 HID 震动字节让位——同一对音圈被 HID 与音频双驱动会叠成浑浊触感（PS5 也只用音频）；布局行以 `out.audio_haptic` 声明能力，Pro 2 的音频接口不在此列（它的反馈走同代透传）。

DualSense 蓝牙输出报告 b1 的高半字节是序号、**每份报告都要递增**（内核 hid-playstation.c 的 `DS_OUTPUT_SEQ_NO`，低半字节 tag 保持 0）——
恒值的报告会被手柄当重复包，蓝牙路径的震动因此时有时无（USB 形态没有该字节）。
布局行以 `seq_off` 声明偏移、编码器在 CRC 前递增写入；DS4 的蓝牙报告头是静态的、没有序号字节。
取舍与实现细节见 [ADR 0042](adr/0042-ds5-audio-haptics-onboard-synthesis.md) 与
[ADR 0043](adr/0043-ds5-bridge-pc-side-audio-haptics.md)。
