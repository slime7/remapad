...
参考实现里还有「无活动超时进休眠、按键唤醒后发 2 秒 0x81 突发」的分支，本设备把它收敛成「常态回连形态 + 显式唤醒窗口」：
未连接期间常发的回连形态（0x00）不叫醒休眠中的主机，醒着的主机会自己连回来；只有用户按下调试页 HOME（未连接时）或串口 `wake` 才开 10 秒窗口发 0x81 把它叫起来。
见 §12 与 [ADR 0031](adr/0031-ns2-reconnect-adv-and-explicit-wake-window.md)。
...
- **回连入口就是回连形态（2026-09-15 复测修正）**：主机停在握把/顺序页、停在首页、以及用电源键从待机醒来三种情况下，带主机地址的 0x00 形态都被采纳并自动连回（设备断开后 25 秒内 `link` 显示 `feat=1` 与输入被采用）；
  ...
  设备侧因此把 0x00 回连形态当作已配对身份的常态广播（未连接期间常驻）：它不会叫醒休眠中的主机；0x81 只在**显式唤醒窗口**内发（10 秒，调试页 HOME 在未连接时或串口 `wake` 打开，主机连上或窗口到期即关闭）。
  配对流程与未配对身份才发发现广播、流程之外静默。取舍见 [ADR 0031](adr/0031-ns2-reconnect-adv-and-explicit-wake-window.md)；
  形态对决保留为诊断开关（串口 `adv auto|wake|reconnect`，auto 为默认），仅用于实机 A/B 对账。
