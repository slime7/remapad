...
| 翻页滑入（每帧） | 48000 px（整个内容框） | 47-99 ms | 17 ms |
| 底栏与模式切换 | 8192-24332 px | 21-48 ms | 5-11 ms |
| 常驻动画（广播呼吸 / 配对转圈） | 2240-2456 px | 12-17 ms | 1.5-2 ms |
| 无变化 | 0 | 不出帧 | — |

首帧一次性付清（启动画面盖住这段时间）；翻页滑入 90 ms 的名义时长在实机上走 5-6 帧、约 290 ms；
常驻动画每 50 ms 一格（状态轮询的节拍），每格约 15 ms；没有任何变化时一帧都不画。
- 代价规则是改界面的硬约束：
  圆角加边框的元素要同时给底色、翻页只在行进侧滑入 16px（每帧重画整个内容框，实测一帧 47-99 ms，时长别再拉长）并让同侧箭头弹一下。
