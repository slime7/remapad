...
扩展口可用信号：
`5V` / `3V3` / `GND`、I2C（`GPIO10` / `GPIO11`）、UART（`GPIO43` / `GPIO44`），以及 `GPIO2` / `GPIO3` / `GPIO17` / `GPIO18`。
...
此外 `pwr_key.c` 负责 PWR 按键（GPIO40 采样，短按息屏 / 长按切连接模式），并在 `app_main` 入口把 SYS_EN（GPIO41）拉高锁存电池供电，USB 供电下该锁存被旁路。
