...
- 配对与连接状态接的是真实 BLE 会话（NimBLE 手柄外设，进度见 [ROADMAP.md](ROADMAP.md)）：
  ...
  配对成功以协议证据判定（初始化 / 0x15 握手完成、凭证匹配回连，或主机在链路上启用特性 0x0c/0x04），NVS 凭证只是重启后仍成立的持久化证据，两者独立；配对六态由此实时推导并经 `pairingStateChanged` 推送。
  第三条证据覆盖两个现场——主机换了随机地址、主机已有本机凭证而不再重跑 0x15：只看地址会把在用的链路一直留在等待态（屏幕停在「配对中…」，「配对新主机」也退不出流程），判据与用例见 `ns2_adv_host_registered()`。
...
- **设备对外只模拟一台 Pro Controller 2**（[ADR 0039](adr/0039-pro-controller-only.md)）：身份枚举 `ns2_identity_t` 只有 `NS2_ID_PRO`，单身份、单连接、单广播实例，报告格式固定 `0x09`（USB 模式 `0x05`）。
  JoyCon 形态（专用输入通道 UUID、`0x07` / `0x08` 报文体、导轨键确认、左右分槽凭证）在 2026-09-16 实机对账后移除：主机只接受 public 地址的广播，一台控制器只有一个 public 地址，左右两只无法各自寻址。
  协议事实留在 [controller.md](controller.md) §4.1 / §5.2 / §12 备查。
  ...
  **广播地址一律是公共伪装地址**：主机只接受 public 地址的广播（2026-09-16 实机对账，见 [controller.md](controller.md) §12），派生静态随机地址的形态只留作串口 `advaddr` 对账开关。
...
- 输入与输出已解耦成三段稳定接口（见 [ADR 0021](adr/0021-input-path-three-stage-layering.md)）：
  ...
  主机振幅是 NS2 LRA 的线性档位，ERM 马达低占空比落在死区，归一时经 `pad_rumble_perceived` 感知重映射（采样音色与 CLI 注入不过表）；
