...
- 广播状态位（厂商数据偏移 0x0B）是主机唯一的唤醒判据，三种形态由 `ns2_adv_payload()` 统一成型（`firmware/main/target/ns2/ns2_adv.c`，主机端用例钉住黄金字节）：发现广播不带主机地址、状态 0x00；回连广播带最近一条凭证里的主机地址、状态 0x00；唤醒广播同样带地址、状态 0x81，只在显式唤醒请求后发约 10 秒并自动回到回连形态（调试页「唤醒 HOME」、串口 `wake`；主机扫描窗口远长于真机 2 秒突发，取 10 秒避免错过）。把 0x81 写进回连广播会让休眠中的主机被每一次回连广播立刻唤醒。
