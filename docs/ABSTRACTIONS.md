...
- BLE manager 负责厂商广播字段、GATT service/characteristic、通知订阅、回连、唤醒和配对状态机；配对凭证通过 NVS 等持久化层保存。
  ...
  凭证按身份分槽、每个身份保留最近 2 条（对齐 controller.md「蓝牙配对信息区结构」的两条存储模型：主机公网地址与私有第二接口地址），按 MAC 覆盖、超出丢弃最旧，配对新主机是追加而非覆盖，因此不常驻「解除配对」入口。
- 广播状态位（厂商数据偏移 0x0B）是主机唯一的唤醒判据，三种形态由 `ns2_adv_payload()` 统一成型（`firmware/main/target/ns2/ns2_adv.c`，主机端用例钉住黄金字节）：
  ...
  **广播时机与超时控制**：广播信号由组装参数统一描述（`ns2_adv_signal_t`：窗口时长 + 前置唤醒突发时长），各情况传参经 `ns2_adv_window_open` 开窗；
  开机与点击 Dock 栏中间图标触发 30 秒信号搜索窗口，窗口内已配对身份前 3 秒发 0x81 唤醒突发叫醒休眠主机，后 27 秒转为 0x00 回连形态等主机连回；
  断连检测组装 30 秒回连信号：全程只发 0x00 回连形态、不带唤醒突发，不把用户主动休眠的主机立刻叫醒；
  未配对身份与配对新主机流程在 30 秒内发射发现广播；
  窗口到期若未建立连接则彻底停止广播发射并关闭 BLE，进入静默省电状态以减少芯片发热量，后续需用户主动点击重开。
  唤醒窗口（10 秒全程唤醒突发）内已配对身份发唤醒形态 0x81 把休眠主机叫起来，未配对身份没有主机可唤醒、退化为发现广播；配对流程期间恒发发现广播（要配的是新主机）。
  形态决策是 `ns2_adv_choose_mode(paired, pairing_requested, window, now)` 纯函数，窗口由 `ns2_adv_window_*` 按组装参数维护。
...
- 配对与连接状态接的是真实 BLE 会话（NimBLE 手柄外设）：
  ...
  配对成功以协议证据判定（初始化 / 0x15 握手完成、凭证匹配回连，或主机在链路上启用特性 0x0c/0x04），
  NVS 凭证只是重启后仍成立的持久化证据，两者独立；配对六态由此实时推导并经 `pairingStateChanged` 推送。
  第三条证据覆盖两个现场——主机换了随机地址、主机已有本机凭证而不再重跑 0x15：
  只看地址会把在用的链路一直留在等待态（屏幕停在「配对中…」，「配对新主机」也退不出流程），判据与用例见 `ns2_adv_host_registered()`。
...
- **设备对外只模拟一台 Pro Controller 2**（[ADR 0039](adr/0039-pro-controller-only.md)）：
  ...
  序列号 / PID / 出厂块（0x13000）按连接身份提供。
  专用输入通道只注册 Pro 那一条（真机在 `0x000E` 上按型号换特征值 UUID，主机按自家型号查这条 UUID 才决定订阅），会话往主机订阅的句柄发通知。
  配色按「机身 / 按键 / 高光 / 握把」四段（0xRRGGBB）配置，对应出厂块 `0x13019` 起的布局，手柄设置页给四款预设、串口 `ctrl` 手工指定；
  变化等价于旧手柄断电、新手柄上电：断开现有连接、重算出厂块，并直接打开连接窗口——主机照回连形态自己连回来读到新颜色，用户不必再按一次连接键（未配对时没有主机可回连，保持静默）。
  未配对时按连接键进配对流程，已配对时按连接键发回连形态。
  **广播地址一律是公共伪装地址**：主机只接受 public 地址的广播（实机对账，见 [controller.md](controller.md)「广播过滤与配对记录」），
  派生静态随机地址的形态只留作串口 `advaddr` 对账开关。
...
- 主机推送的手柄固件更新按「接住数据、逐帧应答、不重启」处理（实机对账，协议见 controller.md「Command 0x0D - 手柄固件更新推送」）：
  ...
  版本抬到 9.9.9 后实机不再弹更新提示，但**是否根治未确认**：主机认的版本号未知、收尾那次通信中断仍在，详见 controller.md「Command 0x0D - 手柄固件更新推送」与 ADR 0032。
...
- 输入与输出已解耦成三段稳定接口（见 [ADR 0021](adr/0021-input-path-three-stage-layering.md)）：
  ...
  声明 HD 触觉的设备把「强震」段以 `hd.pulse_hz` 铺到音圈、「发声」段以 `hd.beep_hz` 铺到扬声器并折进音圈（震动映射为震动、音频映射为音频），
...
- 反馈方向已投递到实体手柄：主机下发的震动 / 玩家 LED / 触觉采样经 ble_session 解析成结构化事件，在反馈监听者里归一到 `pad_feedback_t`。
  ...
  （振幅线性直迁、频率码按 9 位 log2 刻度解成 Hz 后按 `hd` 范围夹取回落），采样音色的强震段铺音圈、发声段铺扬声器——映射在布局内完成，
  渲染结果同时驱动板上合成（USB 直插）与 FEEDBACK 帧（桥接 PC 哑渲染，[ADR 0046](adr/0046-ns-waveform-to-ds5-pcm-hd-haptics.md)）。
  「查找手柄」页的蜂鸣由 0x0A 采样流承载（0x02 播放 / 0x00 停止），同期的参数包只带载波（静止包形态：高频频率 0x1E1、零振幅）——两带强度都要跟着事件进持续帧
  ...
  每 10.67ms 一报），发声段没有独立通道、由 PC 侧折进两侧音圈——两条承载通路对音圈的驱动一致（[ADR 0046](adr/0046-ns-waveform-to-ds5-pcm-hd-haptics.md)）。
