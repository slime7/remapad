...
- 广播状态位（厂商数据偏移 0x0B）是主机唯一的唤醒判据，三种形态由 `ns2_adv_payload()` 统一成型（`firmware/main/target/ns2/ns2_adv.c`，主机端用例钉住黄金字节）：
  ...
  **已配对身份在未连接期间常驻回连形态**：它不叫醒休眠中的主机，醒着的主机会自己连回来（这是开机自动回连的路径）；
  0x81 唤醒形态只在**显式唤醒窗口**（10 秒，调试页 HOME 在未连接时或串口 `wake` 打开，主机连上或窗口到期即关闭）内发；
  未配对身份或配对流程期间发发现广播，配对流程之外静默（真机没配对时不广播），开机若当前形态从未配过任何主机则自动进入配对流程。
  形态决策是 `ns2_adv_choose_mode(paired, pairing_requested, steady)` 与 `ns2_adv_steady_mode(wake_window)` 两个纯函数，窗口由 `ns2_adv_wake_window_*` 维护。
  取舍与诊断开关（`adv auto|wake|reconnect`）见 [ADR 0031](adr/0031-ns2-reconnect-adv-and-explicit-wake-window.md)。
...
- 手柄身份与配对凭证按 `ns2_identity_t`（Pro / JoyCon L / JoyCon R）分槽（`ble_creds`，NVS v2 格式，旧单表记录迁移进 Pro 槽）：
  ...
  未配对的新身份自动进配对流程，已配对身份直接发回连形态等主机回连。配对页「按下 LR」（`pressLr`）是手动兜底，未配对期间固件自动注入 L+R 120ms 并每 3 秒重试。
...
- 配对对外的心智模型是「开机即连接，界面只管配对键」：配过主机就常驻回连形态自动回连（主机醒着停在任意页面都认它，主机睡下不被打扰），从未配过则开机自动进入配对流程，主机侧配对记录在首次连接握手时完成，用户不需要在屏幕上做任何确认动作；
...
- 调试注入是控制面进入数据面的唯一低频通道，采样与编码仍由数据面任务独立完成（`firmware/main/dp/dp_source.c`）：
  ...
  HOME 按实体手柄语义分流（`ns2_adv_home_action()`）：主机已注册会话时就是主页键，只注入按键；未连接时按键到不了主机，转成唤醒请求打开唤醒窗口（10 秒内发 0x81 把它叫起来），按钮文案跟着主机状态走。
