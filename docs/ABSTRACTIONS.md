...
经 50 ms 一轮的状态快照交给界面。
退出有两条路：再按一次同样的组合，或在模式里长按叉键（✕）300 ms（进模式前就按着的不算一次长按）。
退出那一刻还按着的键要等到松开才恢复上行，中间这段继续发中性帧，按住的那一段不会漏给主机。
...
- **底图**：轮播带与底栏的两张 SVG 在构建期光栅化成位图（`ui/assets/*.svg`），运行期只做拷贝与混合；
  轮播带那张画的是「一个卡片步距里的周期」而不是整张卡片，按步距平铺才拼出当前页与两侧露出的卡边。
- **页面组织**：八个页面（发布构建七个，开发构建多一个调试页）在 `App` 里全部常驻，
  `visible` 由 `page` 属性决定（只有当前页与左右邻页在画），切页只挪槽位偏移、没有建树成本；页表与每页的焦点项数写在
  `function focus-count-for` 里（固件按这个行数走焦点）。新增页面在 `app.slint` 的轮播带里加一个槽位并同步该函数。
  新增可聚焦行同样同步它。
- **只改属性，不加节点**：列表长度这类会来回变的东西不要用 `for` 动态增删，写固定的槽位再按属性显示/隐藏；
  颜色与尺寸也不要接过渡动画（`animate` 只用在翻页提示与切页滑行上），过渡期间那片区域每帧都要重画。
- **拖动与切页**：轮播带只保留跟手平移（拖动层 `DragArea` 汇入全局 `Drag`，位移到切页阈值就定住，
  抬手按位移阈值或甩动方向切页，往回滑过抖动下限取消这次换页）；页面自身不做滚动——内容按设计稿一次性摆好。
