...
- **运动字段**：6 个 int16 小端（陀螺 XYZ + 加速 XYZ），原始刻度为加速 8192 计数每 g、陀螺 16 计数每 °/s（标称）；
  布局行用 `accel_per_g` / `gyro_per_dps_x1000` 声明这份刻度，解析段换算到私有格式的统一刻度
  （加速 4096 计数每 g、陀螺 14247 计数每 1000 °/s），轴向按 NS 家族的约定——PS 家族的轴向与符号待实机核对。
- DS3 的字段是另一样子（大端加速加单轴陀螺），不适用这套形态。
...
| 触摸 / 电量 / 耳机 | — | — | 不登记（报文里没有触摸区段，电量要走特性报告查询） |
| 运动字段 | `0x29` | 8 | 大端加速 3 轴 + 单轴陀螺（偏航），与本设备的统一格式不同，本轮不登记 |
...
| PS 家族的运动量程（加速 8192 计数/g、陀螺 16 计数每 °/s） | 加速刻度由 DualSense Edge 静止帧（加速第二轴 8089 计数 ≈ 1 g）印证；陀螺是标称值（Linux 驱动的 `DS_GYRO_RES_PER_DEG_S = 1024` 是它对暴露值的归一系数，原始报告按 int16 满量程 ±2048 °/s 取 16 计数每 °/s），**未实机核对** |
| PS 家族运动的轴向与符号 | 未核对：需要手柄在已知朝向下抓一帧静止数据，再回填轴映射与 `invert_mask` |
| DS3 的运动字段（`0x29` 起 8 字节，大端，加速约 4000 计数/g 的标称值） | 取自公开资料，未实机核对，因此不登记 |
