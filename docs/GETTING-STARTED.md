...
| 工具 / 环境 | 建议版本 | 用途说明 |
| :--- | :--- | :--- |
| **Node.js** | v18 或更高 LTS（实测 v24 通过） | 前端构建环境与包管理驱动基座 |
| **pnpm** | v9 或更高版本（优先推荐包管理器） | 工作区依赖安装与全局任务运行 |
| **Bun** | v1.0 或更高版本（实测 v1.4 通过） | 前端原生快速执行 TypeScript 编译器与打包器 |
...
| **启动开发服务器** | \`pnpm run dev\` | 监听 ui/src，启动 8130 端口模拟器并支持热重载 |
| **全量打包构建** | \`pnpm run build\` | 输出 app.js, app.pak, app.pocket 并同步 app_pocket.h |
...
### 1. 模拟器启动报端口占用 (\`EADDRINUSE 127.0.0.1:8130\`)
