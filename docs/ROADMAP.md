...
- [x] 上报节奏 A/B（主机「查找手柄」）：15 ms 下 66.7 帧/秒、发送失败计数为 0、主机全程保持订阅，震动与操作正常；
...
- [x] 3.5 mm 耳机状态核对（DualSense Edge 蓝牙）：插拔差分定位到第 55 字节（bit0 插入、bit1 带麦），已回填 `pad/layouts/ds5.c`；
  主机接受 0x05 / 0x0D，换上 0x07 / 0x0F 后约 150 ms 取消订阅，因此派生值只报插入，带麦档留待 0x002C 音频通路落地后再评估；DS4 与 DS5 有线行未核对。
- [x] DualSense 蓝牙电量字节核对（DualSense Edge 蓝牙）：两份抓包差分定位到第 54 字节
...
- [ ] 验收：选项卡切换、列表滚动、跑马灯这类动画部位的单帧重绘面积等于该部位本身，滚动帧稳定达到 tick 频率。
  当前整幅帧渲染约 50 ms、滚动 17–20 fps，目标频率见 [adr/0037](adr/0037-ui-tick-rate-30hz.md)。
