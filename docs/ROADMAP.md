...
- **配对与回连**：Command 0x15 四步配对在实机完成且不触发 SMP；主机停在首页或待机时设备上电即自动回连（常驻唤醒广播，不进主机配对页），注入按键主机可见；
  待机主机被唤醒这一副作用的实际体验确认（必要时用 `adv reconnect` 回退）。
- **JoyCon 组合**：
  `link` 确认两个身份各自在发发现广播且地址不同 → 主机 Grip 页搜索并把两只组合成一份配置（未配对期间固件自动注入 L+R 120ms，`key lr` 是手动兜底）→ 分侧注入 `key a`（只右手柄）、
