...
- **JoyCon 组合**：`link` 确认两个身份各自在发发现广播且地址不同 → 主机 Grip 页搜索并把两只组合成一份配置（`key lr` 对应组合确认）→ 分侧注入 `key a`（只右手柄）、`key l`（只左手柄）、`stick l 4095 2048`（左摇杆偏右、右摇杆不动）验证两只各自上报；分侧凭证断连后回连正常。
...
- [ ] 家族表按实测抓包回填：用 `cd pc ; uv run python bridge.py --dump` 抓 Xbox 有线 / Xbox 蓝牙 / PS 蓝牙 / Steam 原生布局的原始报告，核对并修正 `firmware/main/pad/pad_device.c` 家族表里的字段偏移（Steam 原生布局尚未登记，当前走 Xbox 兜底并置能力位标记）。重点核对 Xbox Series 的分享位与 DualSense 的静音位——两处都按公开资料填，抓包前只作初值。
