...
## 阶段补强 7（2026-09 第七批：放弃按需挂载、恢复补挂与垫高回退）　状态：代码完成（UI 预览逐像素比对通过；实机待烧录确认）

- [x] **放弃按需挂载与分帧填充**：删除 `useProgressiveMount` 与各页的 `useMountCursor`，恢复 ADR 0013 的「首帧只挂首页、其余 6 页在首帧之后每帧补挂一页」，切页只翻 `hidden`、页面一次建树完成（见 [ADR 0015](adr/0015-restore-deferred-page-mount-after-first-frame.md)）。
- [x] **移除首页加载提示**：加载文案与 spinner 一并去掉，首页只留两个状态圆钮。
- [x] **垫高回退到 `BottomPlaceholder` 组件**：滚动列末尾重新放占位节点（`BOTTOM_PAD_H = 120`），不再把垫高写进滚动列内边距；内容高度估算仍由各页提供，误差由垫高的兜底余量吸收。
- [x] **手柄设置页去掉类型说明行**：两张类型卡只保留标题，卡片高度回到 py-3 + 标题行，序列号两行结构不变（切换仍不重建节点）。
- [x] **滚动改为声明式**：`usePageScroll(active, scrollable, contentH)` 由调用方一次声明页面能否滚动（首页声明为不可滚动），不再用内容高度推导；`overscroll: 0` 取消橡皮筋，拖到边界即夹住、松手不再回弹。

## 阶段补强 8（2026-09 第八批：滚动手感、垫高与手势注册）　状态：代码完成（UI 预览逐帧追踪与逐像素比对通过；实机已烧录，启动日志正常）

- [x] **抛掷落点越界改写到边界**：`overscroll: 0` 只夹住了拖拽，框架的 fling 撞到边缘仍会转交边缘弹簧（引擎仿真中松手 2500 px/s 会冲过边界 72 px 再弹回）；松手时用 `projectFling` 预判落点，越界就改写成一段到边界的补间，时长按三次缓出的起始速度对齐松手速度，界内仍走原生 fling。
- [x] **手柄设置页高度按形态估算**：类型卡实测 53 px（py-3 上下 24 + text-2xl 图标行 29），旧公式按 42 px 计，多出的高度原本被 120 的垫高兜住；内容高度改为 `contentHeight(joycon)`（Pro 317、JoyCon 339），两种形态滚到底都停在底栏上方 8 px（实测 198，底栏顶 208）。
- [x] **底部垫高 120 → 80**：底栏遮挡 72 之外只留 8 px 余量，滚到底时最后一张卡片仍露在底栏上方，滚动行程随之收短。
- [x] **手势层改为单识别器 + 区域开关**：按官方文档「关掉识别器」的写法，整应用只注册一个纵向手势，由当前接管滚动的页面给出整屏 region，非当前页或不可滚动的页返回 null；不再随切页 dispose 与重注册（官方优先级就是注册顺序，且 dispose 会取消进行中的触点）。
