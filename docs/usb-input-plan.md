...
状态：**两条输入路径的代码都已落地**（桥接：PC 输入 → NS2；USB host 直插：手柄插板卡 → NS2），实机核对待做——USB mux 切换的实机核对未完成，VBUS 供电路径已按 V2.1 原理图确认为外部注入（TP1）。
...
- VBUS 5V 供电路径已按 V2.1 原理图确认：板上没有电池→5V 升压级，电池正极经电源开关 Q5 搭在 VBUS 网络上只有约 3.5–4.1V，点不亮手柄；
  host 供电需从 TP1（TP2 接 GND）注入 5V，注入同时经 ETA6098 给电池充电。
- 桥接路径的 PC 侧配套程序已落地（[pc/README.md](../pc/README.md)），手柄插板卡这条路径的固件实现也已完成；剩下的门禁是 mux 切换的实机核对（VBUS 供电已确认走外部注入，见上一条）。
...
2. **VBUS 供电确认（已完成，V2.1 原理图）**：板上无 5V 输出路径，host 模式给手柄供电需从 TP1 外部注入 5V（与充电输入同网络，注入同时给电池充电）；手柄能否枚举仍要实机验证。
