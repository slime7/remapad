...
固件路线图包含 BLE 配对、用户设置与用户上传 amiibo（NTAG215 二进制），并保留将来 OTA 升级的可能。
原分区表仅有 nvs、phy_init 与 4MB factory，16MB Flash 尾部约 11.7MB 未分配；这些能力在部署后引入都必须改动分区表，
而布局变化会连带擦除已写入的用户数据。当前设备尚无持久化数据，是冻结分区布局的最低成本时机。
...
- 正面：OTA 能力将来可直接使用 esp_ota API 而无需再动分区表；storage 分区约 7.9MB，可存上万个 NTAG215 镜像（单个 540B）并为其他用户数据预留空间。
- 代价：应用空间从单 4MB 变为双 4MB 互备；otadata 位置偏离惯例，需借助分区表注释理解；
  运行期切到 ota_1 后，开发期固定写 0x10000 的 app-flash 未必刷到被启动的分区，需先擦除 otadata 再继续烧录。
