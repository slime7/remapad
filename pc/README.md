...
其中触觉采样字节带原始采样 ID、仅打印展示——采样是主机点播的声音，固件不向
桥接渲染：输入设备有线接入时由板载蜂鸣器按音色表发声，蓝牙手柄直接丢弃），
...
不加限速会把会话循环拖到输入转发卡顿（2026-09-19 实机：蓝牙手柄游戏内约 100 条/秒）。
...
`haptic_synth.c` 一致；触觉采样不进合成（FEEDBACK 的采样字节只供日志展示）。
同时 PC 发固件 CLI `haptic audio on`，设备据此把桥接写回的
