...
- **USB 直插 PC**：对它的 4ch 扬声器端点开 WASAPI 共享流，通道 3/4（RL/RR，直连左右
  ...
  没有真正的声音时两路填充 0 静音。
- **蓝牙连接 PC**：HID 之外没有音频接口，触觉流改走 SAxense 逆向的私有报告 0x32
  （141 字节 = 报文头 + packet 0x11 配置/序号 + packet 0x12 承载 64 字节 PCM + 尾部
  CRC32，种子 0xA2 与 0x31 同一规则），3000Hz / 2 声道 / 8-bit、每 10.67ms 一报由
  发送线程推送，空闲时发静音报文保持通路活跃；蓝牙上没有扬声器通道，发声段随触觉
  子帧铺到音圈。
...
（WASAPI 要秒级），串口只允许主循环一个写者，后台线程只置通知标志。蓝牙的 0x32
发送线程独立于会话主循环（蓝牙 HID 写回慢，约 94 报/秒），写的是会话已打开的
同一 HID 句柄；Windows 对 141 字节写回的接受度待实机核对。
