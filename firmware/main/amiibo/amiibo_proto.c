...
amiibo_upload_result_t amiibo_upload_begin(amiibo_upload_t *up, const uint8_t *payload, size_t len, int64_t now_us)
{
...
amiibo_upload_result_t amiibo_upload_data(amiibo_upload_t *up, const uint8_t *payload, size_t len, int64_t now_us)
{
  ...
  if (data_len == 0 || data_len > AMIIBO_DATA_MAX_LEN || offset + data_len > up->expected || offset > up->received) {
    result.code = AMIIBO_CODE_OFFSET_ERROR;
    return result;
  }
...
amiibo_upload_result_t amiibo_upload_end(amiibo_upload_t *up, int64_t now_us, amiibo_store_fn store, void *user)
{
  (void)now_us;
  amiibo_upload_result_t result = {
    .state = up->state,
...
amiibo_upload_result_t amiibo_upload_tick(amiibo_upload_t *up, int64_t now_us)
{
  ...
  if (up->state != AMIIBO_STATE_RECEIVING || now_us - up->last_rx_us <= AMIIBO_SESSION_TIMEOUT_US) {
    return result;
  }
  result.state = AMIIBO_STATE_FAILED;
