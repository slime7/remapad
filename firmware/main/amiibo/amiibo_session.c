...
static void reply(const amiibo_upload_result_t *result)
{
    ...
    input_link_send_frame_wait(INPUT_FRAME_TYPE_AMIIBO_ACK, 0, payload, len,
                               AMIIBO_ACK_TX_TIMEOUT_MS);
}
...
void amiibo_session_handle_frame(const input_frame_view_t *frame)
{
    ...
    case INPUT_FRAME_TYPE_AMIIBO_DATA:
        ...
        if (result.code != AMIIBO_CODE_OK) {
            ESP_LOGW(TAG, "upload data refused (code=%d, received=%u/%u)", (int)result.code,
                     (unsigned)result.received, (unsigned)NS2_NFC_TAG_SIZE);
        }
        break;
