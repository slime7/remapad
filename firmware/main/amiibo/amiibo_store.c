...
esp_err_t amiibo_store_init(void)
{
  ...
  ESP_LOGI(TAG, "store ready on '" AMIIBO_FS_PARTITION "' (%u/%u slots)", (unsigned)amiibo_store_count(),
           (unsigned)amiibo_store_capacity());
  return ESP_OK;
}
...
{
  if (!s_store.mounted || index >= AMIIBO_SLOTS_MAX || !s_store.used[index] || out == NULL || cap == 0) {
    return false;
  }
  char path[24];
...
{
  if (!s_store.mounted || index >= AMIIBO_SLOTS_MAX || !s_store.used[index] || out == NULL ||
      cap < AMIIBO_RECORD_SIZE) {
    return 0;
  }
  char path[24];
...
esp_err_t amiibo_store_select(size_t index)
{
  ...
  ESP_LOGI(TAG, "selected slot %u '%s' (nfc state 0x%02x)", (unsigned)index, name, ns2_nfc_report_state());
  return ESP_OK;
}
...
{
  (void)user;
  if (!s_store.mounted || s_store.selected < 0 || data == NULL || len != NS2_NFC_TAG_SIZE) {
    return false;
  }
  uint8_t record[AMIIBO_RECORD_SIZE];
