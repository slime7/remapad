...
esp_err_t js_bridge_init(void)
{
  ...
  ns2_session_set_colors(app_config_get()->body_color, app_config_get()->button_color, app_config_get()->accent_color,
...
static void handle_get_system_status(int id)
{
  ...
  snprintf(event, sizeof(event),
           ...
           battery_is_charging() ? "true" : "false", backlight_get(), app_config_get()->screen_on ? "true" : "false",
           real_pairing_state(),
           (ns2_session_waiting_pair() || ns2_session_host_registered()) ? "\"pro-controller-2\"" : "null",
           s_bridge.usb_role_host ? "host" : "device", "true", /* 两个角色都已接入数据面：生效标志恒为真。 */
           (unsigned)ns2_session_player_leds(), dp_ui_active() ? "true" : "false",
           input_link_active() && input_link_pc_connected() ? "true" : "false",
           (long long)(esp_timer_get_time() / 1000LL), (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
...
static void handle_set_backlight(int id, const char *cmd)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"backlightSet\",\"id\":%d,\"brightness\":%u,\"success\":true}", id,
           backlight_get());
  reply_raw(event);
}
...
void js_bridge_connect_key(void)
{
  ...
  js_bridge_submit_command(active ? "{\"t\":\"disconnect\",\"id\":0}" : "{\"t\":\"connect\",\"id\":0}");
}
...
void js_bridge_screen_power(bool on)
{
  ...
  snprintf(event, sizeof(event), "{\"t\":\"screenPowerChanged\",\"on\":%s}", on ? "true" : "false");
  js_bridge_post_event(event);
  ESP_LOGI(TAG, "screen power -> %s (backlight %u%%, %s)", on ? "on" : "off", (unsigned)target, esp_err_to_name(err));
}
...
static void handle_set_screen_power(int id, const char *cmd)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"screenPowerSet\",\"id\":%d,\"on\":%s}", id,
           app_config_get()->screen_on ? "true" : "false");
  reply_raw(event);
}
...
static void handle_set_usb_role(int id, const char *cmd)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"usbRoleSet\",\"id\":%d,\"role\":\"%s\",\"active\":true}", id,
  ...
  snprintf(broadcast, sizeof(broadcast), "{\"t\":\"usbRoleChanged\",\"role\":\"%s\",\"active\":true}",
           want_host ? "host" : "device");
  reply_raw(broadcast);
  ESP_LOGI(TAG, "usb role -> %s (runtime only, reset returns to serial)", want_host ? "host" : "device");
}
...
static void handle_start_pairing(int id)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"pairingResult\",\"id\":%d,\"state\":\"%s\"}", id, real_pairing_state());
  reply_raw(event);
...
static void handle_unpair(int id)
{
  ns2_session_unpair();
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"unpairResult\",\"id\":%d,\"state\":\"%s\"}", id, real_pairing_state());
  reply_raw(event);
...
static void handle_reboot(int id)
{
  ...
  ESP_LOGI(TAG, "reboot requested; restarting in %lld ms", (long long)(REMAPAD_REBOOT_DELAY_US / 1000LL));
}
...
static void handle_debug_key(int id, const char *cmd)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"debugKeySet\",\"id\":%d,\"key\":\"%.*s\"}", id, (int)key_len, key);
  reply_raw(event);
}
...
static void handle_get_controller_config(int id)
{
  ...
  snprintf(event, sizeof(event),
           ...
           "\"gripColor\":%lu},"
           "\"addresses\":{\"pro\":\"%s\"}}",
           id, (unsigned long)cfg->body_color, (unsigned long)cfg->button_color, (unsigned long)cfg->accent_color,
           (unsigned long)cfg->grip_color, pro_mac);
  reply_raw(event);
}
...
static void handle_set_controller_config(int id, const char *cmd)
{
  ...
  snprintf(event, sizeof(event),
           ...
           "\"gripColor\":%lu}}",
           id, (unsigned long)body, (unsigned long)button, (unsigned long)accent, (unsigned long)grip);
  reply_raw(event);
  ESP_LOGI(TAG, "controller colors -> %06lx/%06lx/%06lx/%06lx (persisted)", (unsigned long)body, (unsigned long)button,
           (unsigned long)accent, (unsigned long)grip);
}
...
static void handle_get_ds_behavior(int id)
{
  ...
  snprintf(event, sizeof(event),
           ...
           id, cfg->ds_touchpad_plus_minus ? "true" : "false", cfg->ds_capture_key ? "true" : "false");
  reply_raw(event);
}
...
static void handle_set_ds_behavior(int id, const char *cmd)
{
  ...
  reply_raw(event);
  ESP_LOGI(TAG, "ds behavior -> touchpad +/-=%u capture=%u (persisted)", (unsigned)touchpad_plus_minus,
           (unsigned)capture_key);
}
...
static void pairing_state_poll(void)
{
  ...
  if (s_bridge.last_pairing_state == NULL || strcmp(s_bridge.last_pairing_state, state) != 0) {
    s_bridge.last_pairing_state = state;
    char event[REMAPAD_EVENT_MAX];
    snprintf(event, sizeof(event), "{\"t\":\"pairingStateChanged\",\"state\":\"%s\"}", state);
    reply_raw(event);
...
static void pad_ui_mode_poll(void)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"padUiModeChanged\",\"on\":%s}", on ? "true" : "false");
  reply_raw(event);
...
static void pad_attached_poll(void)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"padAttachedChanged\",\"attached\":%s,\"name\":\"%s\"}",
...
static void pc_link_poll(void)
{
  ...
  char event[REMAPAD_EVENT_MAX];
  snprintf(event, sizeof(event), "{\"t\":\"pcLinkChanged\",\"connected\":%s}", connected ? "true" : "false");
  reply_raw(event);
...
void js_bridge_service(void)
{
  ...
  } else if (s_bridge.power_off_stage == 2 && esp_timer_get_time() >= s_bridge.power_off_at_us) {
...
esp_err_t js_bridge_service_start(void)
{
  if (xTaskCreatePinnedToCore(bridge_service_task, REMAPAD_BRIDGE_TASK_NAME,
                              REMAPAD_BRIDGE_TASK_STACK_BYTES / sizeof(StackType_t), NULL, REMAPAD_BRIDGE_TASK_PRIORITY,
                              NULL, REMAPAD_BRIDGE_TASK_CORE) != pdPASS) {
