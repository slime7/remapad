...
{
    return type == INPUT_FRAME_TYPE_OTA_BEGIN || type == INPUT_FRAME_TYPE_OTA_DATA ||
           type == INPUT_FRAME_TYPE_OTA_END;
}
...
static void reply(const ota_proto_result_t *result, bool with_version)
{
    ...
    const size_t len = ota_proto_encode_ack(result, ota_session_running_version(), with_version,
...
static void handle_begin(const uint8_t *payload, size_t len)
{
    ...
    if (err != ESP_OK) {
        const ota_code_t code = err == ESP_ERR_OTA_ROLLBACK_INVALID_STATE ? OTA_CODE_BUSY
                                                                         : OTA_CODE_FLASH_ERROR;
...
{
    const ota_proto_result_t result = ota_proto_data(&s_ota.proto, payload, len, window_end,
                                                    esp_timer_get_time(), ota_flash_write,
                                                    NULL);
    if (result.reply) {
        reply(&result, false);
    }
...
static void handle_end(void)
{
    ota_proto_result_t result =
        ota_proto_end(&s_ota.proto, esp_timer_get_time(), ota_flash_write, NULL);
    ...
    s_ota.handle_open = false;
    if (err != ESP_OK) {
        const ota_code_t code =
            err == ESP_ERR_OTA_VALIDATE_FAILED ? OTA_CODE_VERIFY_FAILED : OTA_CODE_FLASH_ERROR;
        result = ota_proto_fail(&s_ota.proto, code);
        reply(&result, false);
        s_ota.phase = OTA_PHASE_FAILED;
...
void ota_session_handle_frame(const input_frame_view_t *frame)
{
    ...
    if (frame->payload_len > OTA_DATA_PAYLOAD_MAX) {
        ESP_LOGW(TAG, "ota frame 0x%02x too long (%u bytes), dropped", frame->type,
...
esp_err_t ota_session_start(void)
{
    ...
    s_ota.queue = xQueueCreateWithCaps(OTA_QUEUE_LEN, sizeof(ota_queue_slot_t),
                                       MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (s_ota.queue == NULL) {
        return ESP_ERR_NO_MEM;
    }
    if (xTaskCreate(ota_task, "remapad-ota", OTA_TASK_STACK, NULL, OTA_TASK_PRIO,
                    &s_ota.task) != pdPASS) {
