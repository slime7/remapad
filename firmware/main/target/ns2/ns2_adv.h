...
#define NS2_ADV_SIGNAL_SEARCH                                                                                          \
  ((const ns2_adv_signal_t){ .duration_us = NS2_ADV_CONNECT_WINDOW_US, .burst_us = NS2_ADV_WAKE_BURST_US })

/** 断连回连：主机睡下链路断开后自动开的回连窗口，全程回连形态不带唤醒突发
 *  ——链路断开可能正是用户主动休眠主机，回连不得把它立刻叫醒。 */
#define NS2_ADV_SIGNAL_RECONNECT ((const ns2_adv_signal_t){ .duration_us = NS2_ADV_CONNECT_WINDOW_US, .burst_us = 0 })

/** 唤醒：HOME 唤醒窗口，整窗发唤醒形态把休眠主机叫起来。 */
#define NS2_ADV_SIGNAL_WAKE                                                                                            \
  ((const ns2_adv_signal_t){ .duration_us = NS2_ADV_WAKE_WINDOW_US, .burst_us = NS2_ADV_WAKE_WINDOW_US })

/** 开窗（重新计时）：按组装信号设定窗口时长与唤醒突发，重复开窗按新信号
 *  换算并重新计时。 */
void ns2_adv_window_open(ns2_adv_window_t *win, const ns2_adv_signal_t *signal, int64_t now_us);
...
ns2_adv_mode_t ns2_adv_choose_mode(bool paired, bool pairing_requested, const ns2_adv_window_t *window, int64_t now_us);
...
const uint8_t *ns2_adv_choose_host_mac(const uint8_t *recorded, const uint8_t *const creds[], size_t cred_count);
 ...
 *  「已订阅但没启用特性」的快捷回连不算注册。 */
bool ns2_adv_host_registered(bool addr_matched, bool pair_handshake_done, bool features_enabled);
 ...
 *  NULL 时退化为发现形态。 */
void ns2_adv_payload(uint8_t out[NS2_ADV_PAYLOAD_LEN], uint16_t pid, ns2_adv_mode_t mode, const uint8_t host_mac[6]);
