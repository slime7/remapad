...
/**
 * 主机推送的手柄固件更新：0x0018 上的记录流装配（协议见 controller.md「广播过滤与配对记录」）。
 *
 * 主机把一条 0x0d/0x04 命令帧切成若干记录逐条写入：记录头 4 字节（类型、
 * 序号、载荷长度小端）+ 载荷；载荷按到达顺序拼成命令帧——8 字节帧头（体长
 * 在字节 4-5，大端）+ 帧体。实测一次完整推送是 114 帧、每帧 4108 字节
 * （帧头声明体长 4100，体首 4 字节是块大小 4096）。
 *
 * 本模块只做装配与留样，不解释帧体语义：帧体内容还没逆向出来，伪装应答由
 * ble_session 依据这里的装配结果发出（帧凑齐即回一条空体应答，实测主机接受）。
 */

/** 帧装配缓冲上限：实测一帧 4108 字节，留足余量。 */
