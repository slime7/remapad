...
{
  if (s_out.sink.session_count == NULL || s_out.sink.session_info == NULL || s_out.sink.send_report == NULL) {
    return;
  }
...
{
  if (pad == NULL || pad->raw_len == 0 || s_out.sink.session_count == NULL || s_out.sink.session_info == NULL ||
...
void ns2_rumble_band_strengths(const uint8_t raw[16], uint8_t *lf, uint8_t *hf)
{
  ...
  for (size_t g = 0; g < 3; g++) {
    ...
    const uint8_t group_lf = (uint8_t)(((v >> NS2_KEY_LF_AMP_SHIFT) & NS2_KEY_LF_AMP_MASK) >> 2);
    const uint8_t group_hf = (uint8_t)((v >> 32) & NS2_KEY_HF_AMP_MASK);
    if (group_lf > best_lf) {
      best_lf = group_lf;
    }
    if (group_hf > best_hf) {
...
void ns2_rumble_band_frequencies(const uint8_t raw[16], uint16_t *lf_hz, uint16_t *hf_hz)
{
  ...
  for (size_t g = 0; g < 3; g++) {
    const uint64_t v = key_word(&raw[1 + g * 5]);
    const uint16_t group_lf = key_freq_hz((uint16_t)((v >> NS2_KEY_LF_FREQ_SHIFT) & NS2_KEY_LF_FREQ_MASK));
    const uint16_t group_hf = key_freq_hz((uint16_t)((v >> NS2_KEY_HF_FREQ_SHIFT) & NS2_KEY_HF_FREQ_MASK));
    if (group_lf > best_lf) {
      best_lf = group_lf;
    }
    if (group_hf > best_hf) {
...
bool ns2_rumble_parse(const uint8_t *data, size_t len, ns2_rumble_event_t *out)
{
  ...
  out->left_on = (out->raw[0] & 0x40) != 0 && ns2_rumble_strength(out->raw) > NS2_RUMBLE_CARRIER_MAX;
  out->right_on = (out->raw[16] & 0x40) != 0 && ns2_rumble_strength(&out->raw[16]) > NS2_RUMBLE_CARRIER_MAX;
  return true;
}
