...
static const uint8_t s_motion_capture[40 + 8] = {
  0x06, 0x70, 0x95, 0x5B, 0x34, 0xB6, 0x94, 0x78, 0x00, 0x0D, 0x43, 0xB7, 0xFB, 0x37, 0x42, 0x01,
  0x2C, 0x83, 0xFF, 0x41, 0x34, 0x04, 0x9E, 0x15, 0x0E, 0x88, 0x35, 0x92, 0xCB, 0xCD, 0x53, 0xC3,
  0xA2, 0xBA, 0x49, 0xC3, 0x9F, 0x78, 0x07, 0x51, 0x6C, 0xBE, 0x81, 0x4B, 0x20, 0x54, 0xDF, 0x58,
};
...
{
  return (uint8_t)(((state->battery_level & 0x0F) << 2) | ((state->charging ? 1u : 0u) << 1) |
...
static void buttons_09(const ns2_controller_state_t *state, uint8_t out[3])
{
  ...
  out[0] = (uint8_t)(btn_bit(b, NS2_BTN_RSTICK, 7) | btn_bit(b, NS2_BTN_PLUS, 6) | btn_bit(b, NS2_BTN_ZR, 5) |
                     btn_bit(b, NS2_BTN_R, 4) | btn_bit(b, NS2_BTN_X, 3) | btn_bit(b, NS2_BTN_Y, 2) |
                     btn_bit(b, NS2_BTN_A, 1) | btn_bit(b, NS2_BTN_B, 0));
  out[1] = (uint8_t)(btn_bit(b, NS2_BTN_LSTICK, 7) | btn_bit(b, NS2_BTN_MINUS, 6) | btn_bit(b, NS2_BTN_ZL, 5) |
                     btn_bit(b, NS2_BTN_L, 4) | btn_bit(b, NS2_BTN_DPAD_UP, 3) | btn_bit(b, NS2_BTN_DPAD_LEFT, 2) |
                     btn_bit(b, NS2_BTN_DPAD_RIGHT, 1) | btn_bit(b, NS2_BTN_DPAD_DOWN, 0));
  out[2] = (uint8_t)(btn_bit(b, NS2_BTN_C, 4) | btn_bit(b, NS2_BTN_GL, 3) | btn_bit(b, NS2_BTN_GR, 2) |
                     btn_bit(b, NS2_BTN_CAPTURE, 1) | btn_bit(b, NS2_BTN_HOME, 0));
}
...
{
  const uint32_t b = state->buttons;
  out[0] = (uint8_t)(btn_bit(b, NS2_BTN_ZR, 7) | btn_bit(b, NS2_BTN_R, 6) | btn_bit(b, NS2_BTN_A, 3) |
                     btn_bit(b, NS2_BTN_B, 2) | btn_bit(b, NS2_BTN_X, 1) | btn_bit(b, NS2_BTN_Y, 0));
  out[1] = (uint8_t)(btn_bit(b, NS2_BTN_C, 6) | btn_bit(b, NS2_BTN_CAPTURE, 5) | btn_bit(b, NS2_BTN_HOME, 4) |
                     btn_bit(b, NS2_BTN_LSTICK, 3) | btn_bit(b, NS2_BTN_RSTICK, 2) | btn_bit(b, NS2_BTN_PLUS, 1) |
                     btn_bit(b, NS2_BTN_MINUS, 0));
  out[2] =
      (uint8_t)(btn_bit(b, NS2_BTN_ZL, 7) | btn_bit(b, NS2_BTN_L, 6) | btn_bit(b, NS2_BTN_DPAD_LEFT, 3) |
                btn_bit(b, NS2_BTN_DPAD_RIGHT, 2) | btn_bit(b, NS2_BTN_DPAD_UP, 1) | btn_bit(b, NS2_BTN_DPAD_DOWN, 0));
  out[3] = (uint8_t)(btn_bit(b, NS2_BTN_GL, 1) | btn_bit(b, NS2_BTN_GR, 0));
}
...
static void motion_block(uint8_t *len_out, uint8_t *data_out, size_t cap, const ns2_controller_state_t *state,
                         uint8_t counter)
{
  ...
  *len_out = NS2_INPUT_09_MOTION_LEN;
  if (state->motion_mode == NS2_MOTION_CAPTURE) {
    const size_t n = cap >= sizeof(s_motion_capture) ? sizeof(s_motion_capture) : NS2_INPUT_09_MOTION_LEN;
...
void ns2_encode_input_09(uint8_t out[NS2_INPUT_09_LEN], const ns2_controller_state_t *state, uint8_t counter)
{
  ...
  motion_block(&out[NS2_09_OFF_MOTION_LEN], &out[NS2_09_OFF_MOTION], NS2_INPUT_09_LEN - NS2_09_OFF_MOTION, state,
               counter);
}

void ns2_encode_input_09_usb(uint8_t out[NS2_INPUT_09_LEN + 1], const ns2_controller_state_t *state, uint8_t counter)
{
  out[0] = NS2_REPORT_ID_09;
  ns2_encode_input_09(&out[1], state, counter);
}

void ns2_encode_input_05(uint8_t out[NS2_INPUT_05_LEN], const ns2_controller_state_t *state, uint32_t counter)
{
...
void ns2_encode_input_05_usb(uint8_t out[NS2_INPUT_05_LEN + 1], const ns2_controller_state_t *state, uint32_t counter)
{
