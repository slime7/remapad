...
/** 0x14（读取中）→ 0x15（数据就绪）的模拟耗时：实机对账（2026-09-19）取
...
static struct {
     ...
     *  后）；0x15 = 数据就绪（读卡耗时过后）——2026-09-19 实机对账：主机 0x06
...
esp_err_t ns2_nfc_stage(const uint8_t *data, size_t len)
{
    ...
    ESP_LOGI(TAG, "tag staged: %u bytes sig=%u (polling=%u)", (unsigned)len,
             s_nfc.sig[0] != 0 || s_nfc.sig[31] != 0, s_nfc.polling);
    return ESP_OK;
}
...
{
    if (s_nfc.read_stage == 0x14u &&
        esp_timer_get_time() - s_nfc.read_started_us >= NS2_NFC_STAGE_READY_DELAY_US) {
        return 0x15u;
    }
...
void ns2_nfc_set_report_stage(uint8_t stage)
{
    ...
    s_nfc.drained = false;
    ESP_LOGI(TAG, "report stage -> 0x%02x (nfc state 0x%02x)", s_nfc.read_stage,
...
static size_t build_buffer_read(const uint8_t *req, size_t len, uint8_t *resp, size_t cap)
{
    if (len < NS2_FRAME_HEADER_LEN + 2 ||
        cap < NS2_FRAME_HEADER_LEN + 3 + NS2_NFC_READ_CHUNK_MAX) {
    ...
    if (offset < NS2_NFC_BUFFER_TOTAL) {
        n = NS2_NFC_READ_CHUNK_MAX < NS2_NFC_BUFFER_TOTAL - offset
                ? NS2_NFC_READ_CHUNK_MAX : NS2_NFC_BUFFER_TOTAL - offset;
        if (offset < NS2_NFC_BUFFER_HEADER) {
            uint8_t header[NS2_NFC_BUFFER_HEADER];
            build_read_header(header);
            const size_t in_head = NS2_NFC_BUFFER_HEADER - offset < n
                                       ? NS2_NFC_BUFFER_HEADER - offset : n;
...
static void load_write_buffer(const uint8_t *req, size_t len)
{
    ...
    size_t n = data_len;
    if (offset == 0 && !s_nfc.write_stream && n >= NS2_NFC_WRITE_DESC_LEN &&
        data[0] == 0xD0 && data[1] == 0x07) {
    ...
    if (s_nfc.write_stream) {
        if (s_nfc.write_pos + n > NS2_NFC_TAG_SIZE) {
            ESP_LOGW(TAG, "write stream overflow: pos=%u n=%u",
                     (unsigned)s_nfc.write_pos, (unsigned)n);
            return;
        }
    ...
    ESP_LOGI(TAG, "write buffer +%u bytes (%s, stream pos=%u)", data_len,
             s_nfc.write_stream ? "descriptor stream" : "raw offset",
             (unsigned)s_nfc.write_pos);
}
                          ...
                          size_t cap)
{
    ...
    case 0x05: {
        ...
        uint8_t uid[7];
        if (ns2_nfc_uid(uid)) {
            ESP_LOGI(TAG, "tag info -> uid %02x%02x%02x%02x%02x%02x%02x",
    ...
    case 0x06:
         ...
         * 0x15（数据就绪）跃迁——实机对账（2026-09-19）：主机在 0x06 后检查
         * 一次 0x05 体首字节、随后等报告状态字节到 0x15 才按 0x15 分块拉取。 */
        s_nfc.read_stage = 0x14u;
        s_nfc.drained = false;
        s_nfc.read_started_us = esp_timer_get_time();
        ESP_LOGI(TAG, "host read request (tag %s) -> nfc state 0x%02x",
                 s_nfc.image != NULL ? "present" : "absent", ns2_nfc_report_state());
        return NS2_FRAME_HEADER_LEN;
    case 0x08:
        commit_write_buffer();
        return NS2_FRAME_HEADER_LEN;
    ...
    case 0x15: {
         ...
         * 0x14 会让主机看到「就绪状态消失」而中途放弃抽块（2026-09-19 实机
        ...
        if (offset >= NS2_NFC_BUFFER_TOTAL) {
            ...
            s_nfc.drained = true;
            stage_drain_event();
            ESP_LOGI(TAG, "read drained (nfc state -> 0x%02x, push=%u)", s_drained_state,
                     s_push_mode);
        }
        return built;
    }
