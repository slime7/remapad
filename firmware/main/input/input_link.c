...
static void on_frame(const input_frame_view_t *frame, void *user)
{
  ...
  if (frame->version != INPUT_FRAME_VERSION) {
    ESP_LOGW(TAG, "frame version %u ignored (expected %u)", frame->version, INPUT_FRAME_VERSION);
    return;
  }
  s_frames++;
  ...
  if (frame->type == INPUT_FRAME_TYPE_PING) {
    ...
    input_link_send_frame_wait(INPUT_FRAME_TYPE_PING, 0, &version, sizeof(version), INPUT_LINK_REPLY_TIMEOUT_MS);
    ESP_LOGI(TAG, "bridge ping from PC (protocol v%u)", frame->payload_len > 0 ? frame->payload[0] : 0u);
    return;
  }
...
esp_err_t input_link_start(void)
{
  ...
  s_running = true;
  if (xTaskCreate(input_link_task, "remapad-input", INPUT_LINK_TASK_STACK, NULL, INPUT_LINK_TASK_PRIO, &s_task) !=
      pdPASS) {
    s_running = false;
...
static size_t encode_frame(uint8_t *frame, uint8_t type, uint8_t slot, const uint8_t *payload, size_t payload_len)
{
...
static esp_err_t send_encoded_wait(const uint8_t *frame, size_t len, uint32_t timeout_ms)
{
  ...
  size_t sent = 0;
  while (sent < len) {
    const int n = usb_serial_jtag_write_bytes(&frame[sent], len - sent, pdMS_TO_TICKS(INPUT_LINK_TX_SLICE_MS));
    if (n > 0) {
      sent += (size_t)n;
    }
    if (sent < len && esp_timer_get_time() >= deadline_us) {
      ESP_LOGW(TAG, "frame 0x%02x blocked (%u/%u bytes in %u ms)", frame[3], (unsigned)sent, (unsigned)len,
               (unsigned)timeout_ms);
...
esp_err_t input_link_send_image_info(uint16_t width, uint16_t height, uint32_t timeout_ms)
{
  ...
  const uint8_t payload[INPUT_FRAME_IMAGE_INFO_LEN] = {
    (uint8_t)(width & 0xFFu),           (uint8_t)(width >> 8), (uint8_t)(height & 0xFFu), (uint8_t)(height >> 8),
    INPUT_FRAME_IMAGE_FORMAT_RGB565_LE,
  };
  uint8_t frame[INPUT_FRAME_WIRE_MAX_LEN];
  const size_t len =
      input_frame_encode_wire(frame, sizeof(frame), INPUT_FRAME_TYPE_IMAGE_INFO, 0, 0, payload, sizeof(payload));
  if (len == 0) {
    return ESP_ERR_INVALID_ARG;
  }
  return send_encoded_wait(frame, len, timeout_ms);
...
esp_err_t input_link_send_image_data(uint32_t offset, const uint8_t *data, size_t len, uint32_t timeout_ms)
{
  ...
  const size_t frame_len =
      input_frame_encode_wire(frame, sizeof(frame), INPUT_FRAME_TYPE_IMAGE_DATA, 0, 0, payload, payload_len);
  if (frame_len == 0) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t input_link_send_image_end(uint32_t total_bytes, uint32_t timeout_ms)
{
  ...
  uint8_t frame[INPUT_FRAME_WIRE_MAX_LEN];
  const size_t len =
      input_frame_encode_wire(frame, sizeof(frame), INPUT_FRAME_TYPE_IMAGE_END, 0, 0, payload, sizeof(payload));
  if (len == 0) {
    return ESP_ERR_INVALID_ARG;
  }
  return send_encoded_wait(frame, len, timeout_ms);
...
void input_link_send_host_raw(uint8_t slot, const uint8_t *payload, size_t payload_len)
{
  ...
  uint8_t frame[INPUT_FRAME_WIRE_MAX_LEN];
  const size_t len =
      input_frame_encode_wire(frame, sizeof(frame), INPUT_FRAME_TYPE_HOST_RAW, slot, 0, payload, payload_len);
  if (len == 0) {
    return;
  }
  usb_serial_jtag_write_bytes(frame, len, 0);
