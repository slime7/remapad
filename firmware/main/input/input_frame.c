...
static size_t encode_with_limit(uint8_t *out, size_t out_len, uint8_t type, uint8_t slot, uint8_t seq,
                                const uint8_t *payload, size_t payload_len, size_t max_payload)
{
...
size_t input_frame_encode(uint8_t *out, size_t out_len, uint8_t type, uint8_t slot, uint8_t seq, const uint8_t *payload,
                          size_t payload_len)
{
  return encode_with_limit(out, out_len, type, slot, seq, payload, payload_len, INPUT_FRAME_MAX_PAYLOAD);
}

size_t input_frame_encode_wire(uint8_t *out, size_t out_len, uint8_t type, uint8_t slot, uint8_t seq,
                               const uint8_t *payload, size_t payload_len)
{
  return encode_with_limit(out, out_len, type, slot, seq, payload, payload_len, INPUT_FRAME_WIRE_MAX_PAYLOAD);
}
...
static void process(input_frame_rx_t *rx, input_frame_cb_t on_frame, input_text_cb_t on_text, void *user)
{
  for (;;) {
    ...
    const uint16_t want = (uint16_t)(rx->buf[total - 2] | ((uint16_t)rx->buf[total - 1] << 8));
    if (crc != want) {
...
void input_frame_rx_feed(input_frame_rx_t *rx, const uint8_t *data, size_t len, input_frame_cb_t on_frame,
                         input_text_cb_t on_text, void *user)
{
  size_t offset = 0;
  while (offset < len) {
