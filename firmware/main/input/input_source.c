...
static void update_desc(const pad_report_t *report, bool attached)
{
  ...
  snprintf(s_in.desc, sizeof(s_in.desc), "%s %s %04x:%04x len=%u", pad_family_name(report->family),
           conn_name(report->conn), report->vid, report->pid, (unsigned)report->report_id);
}
