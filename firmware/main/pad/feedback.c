...
static const uint8_t s_perceived[256] = {
  0,   53,  59,  63,  67,  70,  73,  76,  78,  80,  83,  85,  87,  89,  90,  92,  94,  96,  97,  99,  100, 102,
  103, 105, 106, 107, 109, 110, 111, 113, 114, 115, 116, 117, 119, 120, 121, 122, 123, 124, 125, 126, 127, 128,
  129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 143, 144, 145, 146, 147, 148, 149,
  149, 150, 151, 152, 153, 153, 154, 155, 156, 157, 157, 158, 159, 160, 160, 161, 162, 163, 163, 164, 165, 166,
  166, 167, 168, 168, 169, 170, 171, 171, 172, 173, 173, 174, 175, 175, 176, 177, 177, 178, 179, 179, 180, 181,
  181, 182, 182, 183, 184, 184, 185, 186, 186, 187, 187, 188, 189, 189, 190, 191, 191, 192, 192, 193, 194, 194,
  195, 195, 196, 196, 197, 198, 198, 199, 199, 200, 200, 201, 202, 202, 203, 203, 204, 204, 205, 205, 206, 207,
  207, 208, 208, 209, 209, 210, 210, 211, 211, 212, 212, 213, 213, 214, 215, 215, 216, 216, 217, 217, 218, 218,
  219, 219, 220, 220, 221, 221, 222, 222, 223, 223, 224, 224, 225, 225, 226, 226, 227, 227, 228, 228, 228, 229,
  229, 230, 230, 231, 231, 232, 232, 233, 233, 234, 234, 235, 235, 236, 236, 236, 237, 237, 238, 238, 239, 239,
  240, 240, 241, 241, 242, 242, 242, 243, 243, 244, 244, 245, 245, 246, 246, 246, 247, 247, 248, 248, 249, 249,
  249, 250, 250, 251, 251, 252, 252, 252, 253, 253, 254, 254, 255, 255,
};
...
static uint16_t log2_x32(uint32_t x)
{
  ...
  static const uint8_t mantissa[16] = { 0, 3, 5, 8, 10, 13, 15, 17, 19, 21, 22, 24, 26, 27, 29, 31 };
  uint8_t exponent = 0;
...
static void ns1_wave_block(uint8_t out[4], const pad_feedback_t *feedback, size_t side)
{
  ...
  const uint16_t hf_field =
      (uint16_t)((uint16_t)(ns1_freq_code(hf_hz, PAD_NS1_HF_DEFAULT_HZ) - PAD_NS1_HF_CODE_MIN) * 4u);
  const uint8_t lf_field = (uint8_t)(ns1_freq_code(lf_hz, PAD_NS1_LF_DEFAULT_HZ) - PAD_NS1_LF_BASE);
...
static const haptic_env_step_t s_snd_locate[] = {
  { 220, PAD_HAPTIC_PULSE, 0 },   { 400, 0, 0 },  { 500, PAD_HAPTIC_BEEP, 880 }, { 600, 0, 0 },
  { 700, PAD_HAPTIC_BEEP, 1175 }, { 1200, 0, 0 },
};
...
void pad_feedback_hd_render(const pad_layout_t *layout, const pad_feedback_t *feedback, pad_hd_render_t *out)
{
  ...
  for (size_t side = 0; side < PAD_TRIGGER_COUNT; side++) {
    ...
    for (size_t k = 0; k < PAD_HD_KEY_MAX; k++) {
      const pad_rumble_key_t *src = k < count ? &feedback->rumble_keys[side][k] : &feedback->rumble_keys[side][0];
      const bool active = k < count && (src->lf_amp != 0 || src->hf_amp != 0);
      out->key[side][k].lf_freq = active ? hd_freq(hd, src->lf_freq, false) : 0;
      out->key[side][k].lf_gain = active ? hd_gain_apply(hd, (uint8_t)(src->lf_amp >> 2)) : 0;
      out->key[side][k].hf_freq = active ? hd_freq(hd, src->hf_freq, true) : 0;
      out->key[side][k].hf_gain = active ? hd_gain_apply(hd, (uint8_t)(src->hf_amp >> 2)) : 0;
    }
  ...
  if (feedback->haptic_env == PAD_HAPTIC_BEEP && hd->beep_hz != 0) {
    out->speaker.freq = feedback->haptic_tone_hz != 0 ? feedback->haptic_tone_hz : hd->beep_hz;
...
size_t pad_feedback_wire(const pad_feedback_t *feedback, const pad_hd_render_t *hd, uint8_t *out, size_t cap)
{
  ...
  memset(&out[PAD_FEEDBACK_WIRE_LEGACY], 0, PAD_FEEDBACK_WIRE_HD - PAD_FEEDBACK_WIRE_LEGACY);
  for (size_t side = 0; side < PAD_TRIGGER_COUNT; side++) {
...
void pad_feedback_apply(pad_feedback_t *held, uint8_t fields, const pad_feedback_t *event)
{
  if ((fields & PAD_FEEDBACK_FIELD_RUMBLE) != 0) {
    for (size_t side = 0; side < PAD_TRIGGER_COUNT; side++) {
      ...
      memcpy(held->rumble_raw[side], event->rumble_raw[side], sizeof(held->rumble_raw[side]));
      /* 时序子帧随震动事件一起覆盖：HD 映射吃的是它，漏拷会让主机的
             * 波形变化停在旧包络上（与高频带强度同一类陷阱）。 */
      memcpy(held->rumble_keys[side], event->rumble_keys[side], sizeof(held->rumble_keys[side]));
...
bool pad_feedback_equal(const pad_feedback_t *a, const pad_feedback_t *b)
{
  ...
  if (memcmp(a->rumble_on, b->rumble_on, sizeof(a->rumble_on)) != 0 ||
      ...
      memcmp(a->rumble_hf_strength, b->rumble_hf_strength, sizeof(a->rumble_hf_strength)) != 0 ||
...
static size_t encode_report(pad_conn_t conn, uint16_t vid, uint16_t pid, const pad_feedback_t *feedback, uint8_t *out,
                            size_t out_len, bool quiet)
{
  s_last_layout = NULL;
  ...
  if (desc->len == 0 || desc->len > out_len || (!desc->no_report_id && desc->report_id == 0)) {
    s_last_layout = NULL;
    return 0;
  }
  ...
  if (desc->rumble_style == PAD_RUMBLE_NS1_WAVE) {
    ...
    for (size_t side = 0; side < PAD_TRIGGER_COUNT; side++) {
      ...
      const uint8_t source = desc->rumble_band[side] == PAD_RUMBLE_HF ? feedback->rumble_hf_strength[side]
