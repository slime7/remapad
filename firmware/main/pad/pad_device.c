...
static uint16_t apply_stick_deadzone(uint16_t value)
{
  ...
  if (delta > 0) {
    delta = ((delta - PAD_STICK_DEADZONE) * center + (center - PAD_STICK_DEADZONE) / 2) / (center - PAD_STICK_DEADZONE);
    if (delta > center - 1) {
      delta = center - 1;
    }
  } else {
...
static void parse_buttons(const pad_report_t *report, const pad_layout_t *layout, pad_state_t *state)
{
  uint32_t buttons = 0;
  ...
  if (layout->back_off != PAD_OFF_NONE && layout->back_off != 0 && layout->back_map != NULL &&
      range_ok(report, layout->back_off, 1) &&
      !(layout->back_mode_off != PAD_OFF_NONE && range_ok(report, layout->back_mode_off, 1) &&
...
static void parse_axes(const pad_report_t *report, const pad_layout_t *layout, pad_state_t *state)
{
  uint16_t raw[PAD_AXIS_COUNT] = { PAD_AXIS_CENTER, PAD_AXIS_CENTER, PAD_AXIS_CENTER, PAD_AXIS_CENTER };
  if (layout->stick_style == PAD_STICK_U12) {
    /* 左右各三字节：X 与 Y 共用打包块，因此按轴对读取（块起点写在 LX / RX）。 */
    static const size_t pairs[2][2] = { { PAD_AXIS_LX, PAD_AXIS_LY }, { PAD_AXIS_RX, PAD_AXIS_RY } };
    ...
    for (size_t axis = 0; axis < PAD_AXIS_COUNT; axis++) {
      ...
      if (layout->stick_style == PAD_STICK_I16) {
        ...
        if (range_ok(report, off, 2)) {
          raw[axis] = norm_u16((uint16_t)((uint16_t)report->data[off] | ((uint16_t)report->data[off + 1] << 8)));
        }
  ...
  for (size_t i = 0; i < PAD_TRIGGER_COUNT; i++) {
    ...
    if (layout->trigger_style == PAD_TRIGGER_U10) {
      ...
      if (range_ok(report, off, 2)) {
        uint32_t value = (uint32_t)((uint16_t)report->data[off] | ((uint16_t)report->data[off + 1] << 8));
        if (value > 1023u) {
          value = 1023u;
        }
 ...
 */
static void map_axis(int16_t out[3], const int16_t src[3], const uint8_t map[3], uint8_t invert)
{
...
static void parse_motion(const pad_report_t *report, const pad_layout_t *layout, pad_state_t *state)
{
  ...
  map_axis(state->motion.gyro, gyro, layout->motion.gyro_src, (uint8_t)(layout->motion.invert_mask & 0x07u));
  map_axis(state->motion.accel, accel, layout->motion.accel_src, (uint8_t)((layout->motion.invert_mask >> 3) & 0x07u));
  /* 换算到私有格式的统一刻度：布局行声明设备的原始刻度，NS 家族与统一刻度相同。 */
  for (size_t i = 0; i < 3; i++) {
    state->motion.gyro[i] =
        scale_motion(state->motion.gyro[i], PAD_MOTION_GYRO_PER_DPS_X1000, layout->motion.gyro_per_dps_x1000);
    state->motion.accel[i] = scale_motion(state->motion.accel[i], PAD_MOTION_ACCEL_PER_G, layout->motion.accel_per_g);
  }
 ...
 * 归一量程的中点，设备分辨率只参与归一。
 */
static void parse_touch(const pad_report_t *report, const pad_layout_t *layout, pad_state_t *state)
{
  if (layout->touch_max_x == 0 || layout->touch_max_y == 0 ||
      !range_ok(report, layout->touch_off, (uint8_t)(PAD_TOUCH_POINT_SIZE * PAD_TOUCH_POINTS))) {
    return;
  }
  ...
  for (size_t i = 0; i < PAD_TOUCH_POINTS; i++) {
    ...
    pad_touch_t *touch = &state->touch[x < PAD_AXIS_CENTER ? PAD_TOUCH_LEFT : PAD_TOUCH_RIGHT];
    if (touch->pressed) {
      continue;
    }
    touch->pressed = true;
...
static void parse_battery(const pad_report_t *report, const pad_layout_t *layout, pad_state_t *state)
{
 ...
 */
static void parse_headset(const pad_report_t *report, const pad_layout_t *layout, pad_state_t *state)
{
...
void pad_state_from_report(const pad_report_t *report, pad_state_t *state)
{
  ...
  if (layout->native_lang != PAD_LANG_NONE && report->report_id != 0 && report->report_id == report->data[0] &&
      report->len <= PAD_RAW_MAX) {
