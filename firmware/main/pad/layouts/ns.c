 ...
 * 报文体），以及伪装成 NS 布局的第三方手柄；行里的 native_lang 决定能否原样转发给同代目标。
 * 偏移与键位按公开的 HID 报告描述整理；USB 形态首字节是 Report ID，各行偏移比报文体自身大 1。
 */
 ...
 * （0x00 0x01）、后两字节是高频与低频振幅（0x40 为满量程）；私有 0-255 强度按 0x40 缩放，0 即静音。
 * 编码取自公开实现；NS1 的玩家灯与控制灯本轮不映射。
 */
