...
static void cli_status(void)
{
  ...
  snprintf(line, sizeof(line),
           ...
           js_bridge_pairing_state(), cfg->usb_role == APP_CONFIG_USB_HOST ? "host" : "device",
           (unsigned)backlight_get(), (unsigned)cfg->screen_on, (long long)(esp_timer_get_time() / 1000000LL),
           (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
           (unsigned)heap_caps_get_total_size(MALLOC_CAP_INTERNAL), (unsigned)battery_get_voltage_mv(),
           (unsigned)battery_get_percentage(), battery_is_charging() ? 1u : 0u, ota_session_running_version(),
           ota_session_running_partition(), ota_session_state_name(), dp_ui_active() ? "on" : "off",
...
{
  char line[128];
  snprintf(line, sizeof(line), "fw=%s part=%s image=%s ota=%s", ota_session_running_version(),
           ota_session_running_partition(), ota_session_pending_verify() ? "pending-verify" : "confirmed",
...
{
  if (arg[0] == '\0') {
    cli_print(dp_ui_active() ? "pad ui mode on (dpad moves, circle confirms)" : "pad ui mode off");
    return;
  }
  if (strcmp(arg, "on") == 0) {
...
static void cli_stick(const char *arg)
{
  ...
  int x = 0;
  int y = 0;
  if (sscanf(arg, "%7s %7s %7s", side, axis_x, axis_y) < 3 || (strcmp(side, "l") != 0 && strcmp(side, "r") != 0) ||
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (status.connected) {
      ...
      if (status.notify_05 || status.notify_priv) {
        snprintf(notify, sizeof(notify), "%s%s", status.notify_05 ? "05" : "", status.notify_priv ? "P" : "");
      }
      snprintf(line, sizeof(line),
               ...
               ns2_identity_name(status.identity), link_state_name(status.state), status.conn_handle, status.conn_itvl,
               (unsigned)mtu, enc ? 1u : 0u, status.report_format, notify, status.notify_priv_handle,
               status.features_enabled ? 1u : 0u, (unsigned long)status.reports, (unsigned long)tx_fail, tx_rc,
               (unsigned)status.creds, link_adv_name(&status), addr, ns2_output_motion_mode(), ns2_session_ltk_form());
    } else {
      snprintf(line, sizeof(line), "  %-4s %s creds=%u adv=%s addr=%s", ns2_identity_name(status.identity),
               link_state_name(status.state), (unsigned)status.creds, link_adv_name(&status), addr);
    }
    cli_print(line);
  }
}
...
static void cli_ctrl(const char *arg)
{
  ...
  if (arg[0] == '\0') {
    char line[128];
    snprintf(line, sizeof(line), "ctrl body=0x%06x button=0x%06x accent=0x%06x grip=0x%06x", (unsigned)cfg->body_color,
             (unsigned)cfg->button_color, (unsigned)cfg->accent_color, (unsigned)cfg->grip_color);
    cli_print(line);
    return;
  }
  char body_text[16] = { 0 };
  ...
  char grip_text[16] = { 0 };
  const int fields = sscanf(arg, "%15s %15s %15s %15s", body_text, button_text, accent_text, grip_text);
  uint32_t colors[4] = { cfg->body_color, cfg->button_color, cfg->accent_color, cfg->grip_color };
  ...
  char line[96];
  snprintf(line, sizeof(line), "ok ctrl body=0x%06x button=0x%06x accent=0x%06x grip=0x%06x", (unsigned)colors[0],
           (unsigned)colors[1], (unsigned)colors[2], (unsigned)colors[3]);
  cli_print(line);
}
...
static void cli_ds(const char *arg)
{
  ...
  if (changing) {
    ...
    if (key_len >= sizeof(key) || (strcmp(value, "on") != 0 && strcmp(value, "off") != 0)) {
  ...
  char line[96];
  snprintf(line, sizeof(line), "ds touchpad=%s capture=%s%s", touchpad_plus_minus ? "on" : "off",
           capture_key ? "on" : "off", changing ? " (persisted)" : "");
  cli_print(line);
}
...
static void cli_advaddr(const char *arg)
{
  char line[128];
  if (arg[0] == '\0') {
    snprintf(line, sizeof(line), "adv addr form: %s", ns2_adv_addr_form_name(ns2_session_adv_addr_form()));
    cli_print(line);
    return;
  }
  uint8_t value = 0xFF;
...
static void cli_advpdu(const char *arg)
{
  char line[64];
  if (arg[0] == '\0') {
    snprintf(line, sizeof(line), "adv pdu form: %s", ble_controller_adv_pdu_form_name(ble_controller_adv_pdu_form()));
    cli_print(line);
    return;
  }
  uint8_t value = 0xFF;
...
static void cli_report(void)
{
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    const uint8_t fmt = status.report_format == NS2_REPORT_ID_05 ? NS2_REPORT_ID_05 : NS2_REPORT_ID_09;
    ...
    snprintf(line, sizeof(line),
             ...
             ns2_identity_name(status.identity), status.report_format, body[0], body[1], body[2], body[3], body[4],
             body[5], body[6], body[7], body[8], body[9], body[10], body[0x0B], body[0x0C], body[0x0E]);
    cli_print(line);
    printed = true;
  }
  if (!printed) {
...
static void cli_headset(const char *arg)
{
  ...
  if (arg[0] == '\0') {
    ...
    if (ns2_output_headset_override(&override_value)) {
      snprintf(line, sizeof(line), "headset override 0x%02x (auto value 0x%02x)", (unsigned)override_value,
               (unsigned)ns2_output_headset_derived());
    } else {
      snprintf(line, sizeof(line), "headset auto (value 0x%02x)", (unsigned)ns2_output_headset_derived());
    }
    cli_print(line);
    return;
  }
...
static void cli_feedback_state(void)
{
  ...
  char line[144];
  snprintf(line, sizeof(line), "feedback rumble l=%u r=%u hf l=%u r=%u lamp=0x%x haptic=%s bridge-audio=%s",
           (unsigned)held.rumble_strength[PAD_TRIGGER_L2], (unsigned)held.rumble_strength[PAD_TRIGGER_R2],
           (unsigned)held.rumble_hf_strength[PAD_TRIGGER_L2], (unsigned)held.rumble_hf_strength[PAD_TRIGGER_R2],
           (unsigned)held.player_led, haptic, dp_plane_bridge_audio_active() ? "on" : "off");
  cli_print(line);
}
...
static void cli_rumble(const char *arg)
{
  ...
  if (left_end == left || *left_end != '\0' || l > 255 || right_end == right || *right_end != '\0' || r > 255) {
    cli_print("err usage: rumble [off|<l 0-255> <r 0-255>]");
    return;
  }
...
static void cli_pad(void)
{
  ...
  snprintf(line, sizeof(line), "pad %s %s %s %04x:%04x row=0x%02x native=%u fallback=%u relay=%u", source,
           pad_family_name(family), cli_conn_name(conn), (unsigned)vid, (unsigned)pid,
           layout != NULL ? (unsigned)layout->report_id : 0u, layout != NULL ? (unsigned)layout->native_lang : 0u,
           layout == NULL ? 1u : 0u, target_relay_enabled() ? 1u : 0u);
  cli_print(line);
}
...
static void cli_usb(void)
{
  ...
  snprintf(line, sizeof(line), "usb role=%s stack=%u device=%s reports=%lu outputs=%lu console=%s",
           app_config_get()->usb_role == APP_CONFIG_USB_HOST ? "host" : "device", usb_host_running() ? 1u : 0u,
           usb_input_device_desc(), (unsigned long)usb_input_report_count(), (unsigned long)usb_input_output_count(),
...
static void cli_capture(const char *arg)
{
  ...
  char line[64];
  snprintf(line, sizeof(line), "capture %s pushed=%lu dropped=%lu", dp_capture_enabled() ? "on" : "off",
           (unsigned long)pushed, (unsigned long)dropped);
  cli_print(line);
}
...
static void cli_fwver(const char *arg)
{
  ...
  const app_config_t *cfg = app_config_get();
  if (arg[0] == '\0') {
    snprintf(line, sizeof(line), "handset fw %u.%u.%u (reported to host)", cfg->fw_version[0], cfg->fw_version[1],
             cfg->fw_version[2]);
    cli_print(line);
    return;
  }
...
static void cli_fwapply(const char *arg)
{
  char line[64];
  if (arg[0] == '\0') {
    snprintf(line, sizeof(line), "fw upgrade reboot armed=%u", ns2_session_fw_restart_armed() ? 1u : 0u);
    cli_print(line);
    return;
  }
  if (strcmp(arg, "on") == 0) {
...
static void cli_amiibo(const char *arg)
{
  ...
  if (fields < 1) {
    ...
    snprintf(line, sizeof(line), "amiibo slots=%u/%u selected=%d '%s' nfc=0x%02x poll=%u hdr=%u",
             (unsigned)amiibo_store_count(), (unsigned)amiibo_store_capacity(), selected, name,
             (unsigned)ns2_nfc_report_state(), ns2_nfc_polling() ? 1u : 0u, (unsigned)ns2_nfc_header_mode());
    cli_print(line);
    return;
  }
  if (strcmp(cmd, "list") == 0) {
    for (size_t i = 0; i < amiibo_store_capacity(); i++) {
      ...
      snprintf(line, sizeof(line), "  %u%c %s uid=%s", (unsigned)i, amiibo_store_selected() == (int)i ? '*' : ' ', name,
               uid);
      cli_print(line);
    }
  ...
  if (strcmp(cmd, "select") == 0) {
    ...
    if (err != ESP_OK) {
      snprintf(line, sizeof(line), "err amiibo select %d failed: %s", index, esp_err_to_name(err));
      cli_print(line);
      return;
    }
  ...
  if (strcmp(cmd, "del") == 0) {
    ...
    if (err != ESP_OK) {
      snprintf(line, sizeof(line), "err amiibo del %d failed: %s", index, esp_err_to_name(err));
      cli_print(line);
      return;
    }
  ...
  if (strcmp(cmd, "poll") == 0) {
    if (strcmp(sub, "on") == 0 || strcmp(sub, "off") == 0) {
      ...
      snprintf(line, sizeof(line), "ok amiibo polling %s (nfc state 0x%02x)", sub, (unsigned)ns2_nfc_report_state());
      cli_print(line);
      return;
    }
  ...
  if (strcmp(cmd, "state") == 0) {
    if (sub[0] == '\0') {
      snprintf(line, sizeof(line), "ok amiibo nfc state 0x%02x (poll=%u)", (unsigned)ns2_nfc_report_state(),
               ns2_nfc_polling() ? 1u : 0u);
      cli_print(line);
      return;
    }
    const uint32_t stage = strtoul(sub, NULL, 16);
    ns2_nfc_set_report_stage((uint8_t)stage);
    snprintf(line, sizeof(line), "ok amiibo stage -> %s (nfc state 0x%02x)", sub, (unsigned)ns2_nfc_report_state());
    cli_print(line);
    return;
  }
