...
esp_err_t console_out_use_uart0(void)
{
  ...
  esp_err_t err = uart_driver_install(CONSOLE_UART_NUM, CONSOLE_UART_BUF, CONSOLE_UART_BUF, 0, NULL, 0);
  if (err != ESP_OK) {
    return err;
  }
  ESP_ERROR_CHECK(uart_param_config(CONSOLE_UART_NUM, &config));
  ESP_ERROR_CHECK(uart_set_pin(CONSOLE_UART_NUM, CONSOLE_UART_TX_GPIO, CONSOLE_UART_RX_GPIO, UART_PIN_NO_CHANGE,
                               UART_PIN_NO_CHANGE));
  ...
  s_uart_active = true;
  ESP_LOGI(TAG, "console moved to UART0 (GPIO%d/%d, %d baud)", CONSOLE_UART_TX_GPIO, CONSOLE_UART_RX_GPIO,
           CONSOLE_UART_BAUD);
  return ESP_OK;
}
