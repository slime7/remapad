...
/**
 * PWR 按键与电源保持（SYS_OUT=GPIO40 为按键电平，SYS_EN=GPIO41 为电源保持脚）。
 * 事件在 pwr-key 任务上下文回调（内部 RAM 栈，可调用背光/队列接口，但不得触碰 PocketJS guest）：
 * 短按（<600ms 释放）切息屏；长按（3-6s 释放）是连接键；按住超过 6s 让位硬件电源行为、不产生软件事件。
 */
