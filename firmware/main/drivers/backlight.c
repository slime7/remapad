...
esp_err_t backlight_set(uint8_t brightness_pct)
{
  ...
  ESP_RETURN_ON_ERROR(ledc_set_duty(LEDC_LOW_SPEED_MODE, REMAPAD_BACKLIGHT_LEDC_CHANNEL, duty), TAG,
                      "set LEDC duty failed");
  ESP_RETURN_ON_ERROR(ledc_update_duty(LEDC_LOW_SPEED_MODE, REMAPAD_BACKLIGHT_LEDC_CHANNEL), TAG,
                      "update LEDC duty failed");
