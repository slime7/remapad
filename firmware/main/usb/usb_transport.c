...
static bool report_desc_is_gamepad(const uint8_t *desc, size_t len)
{
  for (size_t i = 0; i + 3 < len; i++) {
    if (desc[i] == 0x05 && desc[i + 1] == USB_HID_USAGE_PAGE_GENERIC_DESKTOP && desc[i + 2] == 0x09 &&
...
static bool probe_interface(usb_device_handle_t dev, uint8_t iface_num, uint8_t ep_in, uint8_t ep_out, uint16_t mps_in,
                            uint16_t mps_out)
{
  if (ep_in == 0) {
    return false;
...
static void open_device(uint8_t addr)
{
  ...
  if (!find_gamepad_interface(dev, cfg)) {
    ESP_LOGW(TAG, "device %04x:%04x has no HID gamepad interface (skipped)", (unsigned)dev_desc->idVendor,
...
esp_err_t usb_host_start(void)
{
  ...
  if (xTaskCreate(lib_task, "remapad-usb", USB_LIB_TASK_STACK, NULL, USB_LIB_TASK_PRIO, &s_host.lib_task) != pdPASS) {
    usb_host_uninstall();
  ...
  s_host.running = true;
  if (xTaskCreate(client_task, "remapad-usbc", USB_CLIENT_TASK_STACK, NULL, USB_CLIENT_TASK_PRIO,
                  &s_host.client_task) != pdPASS) {
    usb_host_stop();
    return ESP_ERR_NO_MEM;
  }
