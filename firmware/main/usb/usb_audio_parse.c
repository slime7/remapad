...
{
    return g->is_as && g->alt != 0 && g->pcm && g->format_ok && g->channels > 0 &&
           g->subframe == 2 && g->bits == 16 && g->rate != 0 && g->ep_addr != 0;
}
...
bool usb_audio_find_as_out(const uint8_t *config_desc, size_t len, usb_audio_as_out_t *out)
{
    ...
    while (p + 2 <= end) {
        ...
        if (desc_type == USB_DESC_INTERFACE && desc_len >= 9) {
            ...
            g.ep_addr = 0;
            g.ep_mps = 0;
        } else if (desc_type == USB_DESC_CS_INTERFACE && g.is_as && g.alt != 0 &&
                   desc_len >= 3) {
