...
static void osc_prepare_side(synth_osc_t *lf, synth_osc_t *hf, bool open, const haptic_synth_state_t *state,
                             const haptic_synth_params_t *params, size_t side, size_t key)
{
  uint16_t lf_freq = 0;
  uint8_t lf_gain = 0;
  uint16_t hf_freq = 0;
...
void haptic_synth_fill(haptic_synth_state_t *state, const haptic_synth_params_t *params, int16_t *pcm, size_t frames)
{
  ...
  const uint16_t speaker_target = (uint16_t)(speaker_target32 > 65535u ? 65535u : speaker_target32);
  const int32_t speaker_harm = speaker_freq != 0 && (uint32_t)speaker_freq * 2u < HAPTIC_SYNTH_RATE_HZ / 2u
  ...
  for (size_t side = 0; side < 2; side++) {
    ...
    for (size_t k = 0; k < PAD_HD_KEY_MAX && k < keys; k++) {
      if (params->tones.key[side][k].lf_gain != 0 || params->tones.key[side][k].hf_gain != 0) {
        coil_open[side] = true;
  ...
  synth_osc_t hf[2];
  for (size_t side = 0; side < 2; side++) {
    osc_prepare_side(&lf[side], &hf[side], coil_open[side], state, params, side, key);
  }
  for (size_t i = 0; i < frames; i++) {
    ...
    if (speaker_target != 0 || state->speaker_env != 0) {
      ...
      if (speaker_target != 0) {
        ...
        state->speaker_env = (uint16_t)(env > speaker_target ? speaker_target : env);
      } else {
        state->speaker_env = state->speaker_env > 98u ? (uint16_t)(state->speaker_env - 98u) : 0u;
      }
      if (state->speaker_env != 0) {
        ...
        speaker_sample = ((wave * params->amp_peak) >> 15) * (int32_t)state->speaker_env >> 16;
    ...
    for (size_t side = 0; side < 2; side++) {
      ...
      if (coil_open[side]) {
        ...
        state->coil_env[side] =
            state->coil_env[side] > SYNTH_COIL_FALL ? (uint16_t)(state->coil_env[side] - SYNTH_COIL_FALL) : 0u;
      }
      int32_t sample = 0;
