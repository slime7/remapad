                      ...
                      uint16_t vid, uint16_t pid)
{
  ...
  if (as.sample_rate_hz != HAPTIC_SYNTH_RATE_HZ || as.channels != HAPTIC_SYNTH_CHANNELS || as.subframe_size != 2 ||
      as.bit_resolution != 16 || as.ep_mps < USB_AUDIO_PACKET_BYTES) {
    ESP_LOGW(TAG, "audio format not drivable: %uch %ubit %uHz mps %u", (unsigned)as.channels,
             (unsigned)as.bit_resolution, (unsigned)as.sample_rate_hz, (unsigned)as.ep_mps);
    return false;
  }
  ...
  for (int i = 0; i < USB_AUDIO_XFER_COUNT && ok; i++) {
    if (usb_host_transfer_alloc(USB_AUDIO_PACKET_BYTES * USB_AUDIO_ISOC_PACKETS, USB_AUDIO_ISOC_PACKETS,
                                &s_aud.xfer[i]) != ESP_OK) {
      s_aud.xfer[i] = NULL;
      ok = false;
      break;
  ...
  ESP_LOGI(TAG, "audio haptics streaming: iface %u alt %u ep 0x%02x mps %u", (unsigned)as.iface, (unsigned)as.alt,
           (unsigned)as.ep_addr, (unsigned)as.ep_mps);
  return true;
}
