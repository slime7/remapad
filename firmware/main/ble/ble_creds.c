 ...
 * （读回缓冲区如果按当前长度开、比盘上的短，整张表读不进来，
 * 会话回落去读 v1 老表，拿到的是过期记录）。 */
#define CREDS_BLOB_LEN_MAX (CREDS_BLOB_LEN > CREDS_V1_LEN_MAX ? CREDS_BLOB_LEN : CREDS_V1_LEN_MAX)
...
static void commit_task(void *param)
{
  ...
  while (xQueueReceive(s_commit_queue, &msg, portMAX_DELAY) == pdTRUE) {
    ...
    if (set != ESP_OK) {
      ...
      ESP_LOGI(TAG, "committed host address %02x:%02x:%02x:%02x:%02x:%02x", msg.payload[0], msg.payload[1],
...
static void seed_host_from_creds(void)
{
  for (size_t id = 0; id < NS2_ID_COUNT; id++) {
    if (!s_creds.host_valid[id] && s_creds.count[id] > 0) {
      memcpy(s_creds.host_mac[id], s_creds.records[id][s_creds.count[id] - 1].mac, NS2_CREDS_MAC_LEN);
...
esp_err_t ble_creds_init(void)
{
  ...
  if (s_lock == NULL || s_commit_queue == NULL || xTaskCreate(commit_task, "blecreds", 4096, NULL, 2, NULL) != pdPASS) {
  ...
  if (get == ESP_OK && len >= CREDS_SLOT_LEN) {
    ...
    ESP_LOGI(TAG, "loaded creds (pro=%u, blob %uB)", (unsigned)s_creds.count[NS2_ID_PRO], (unsigned)len);
  } else {
    if (get == ESP_OK) {
    ...
    if (get_v1 == ESP_OK && len >= 1 && (len - 1) % (NS2_CREDS_MAC_LEN + NS2_CREDS_LTK_LEN) == 0) {
      ...
      ESP_LOGI(TAG, "migrated %u legacy pairing record(s) -> pro", (unsigned)s_creds.count[NS2_ID_PRO]);
    }
  }
  ...
  if (nvs_get_blob(handle, CREDS_KEY_HOST, host, &len) == ESP_OK && len >= NS2_CREDS_MAC_LEN) {
    ...
    ESP_LOGI(TAG, "loaded host address %02x:%02x:%02x:%02x:%02x:%02x", host[0], host[1], host[2], host[3], host[4],
             host[5]);
  }
  nvs_close(handle);
  return ESP_OK;
...
{
  size_t count = 0;
  if (identity < NS2_ID_COUNT && s_lock != NULL && xSemaphoreTake(s_lock, portMAX_DELAY) == pdTRUE) {
                         ...
                         const uint8_t ltk[NS2_CREDS_LTK_LEN])
{
  ...
  for (size_t i = 0; i < s_creds.count[identity]; i++) {
    if (memcmp(s_creds.records[identity][i].mac, mac, NS2_CREDS_MAC_LEN) == 0) {
      ...
      xSemaphoreGive(s_lock);
      ESP_LOGI(TAG, "pairing record updated (id=%u total %u)", (unsigned)identity, (unsigned)s_creds.count[identity]);
  ...
  serialize_locked(blob);
  xSemaphoreGive(s_lock);
  ESP_LOGI(TAG, "pairing record saved (id=%u total %u)", (unsigned)identity, (unsigned)s_creds.count[identity]);
  enqueue_commit(CREDS_COMMIT_CREDS, blob, CREDS_BLOB_LEN);
  return ESP_OK;
}
...
void ble_creds_note_host_mac(ns2_identity_t identity, const uint8_t mac[NS2_CREDS_MAC_LEN])
{
  ...
  const bool same = s_creds.host_valid[identity] && memcmp(s_creds.host_mac[identity], mac, NS2_CREDS_MAC_LEN) == 0;
  if (!same) {
    ...
    xSemaphoreGive(s_lock);
    ESP_LOGI(TAG, "host address noted (id=%u %02x:%02x:%02x:%02x:%02x:%02x)", (unsigned)identity, mac[0], mac[1],
...
{
  if (identity >= NS2_ID_COUNT || out == NULL || s_lock == NULL || xSemaphoreTake(s_lock, portMAX_DELAY) != pdTRUE) {
    return false;
  }
