...
static bool aes_ecb_block(const uint8_t key[16], const uint8_t in[16], uint8_t out[16])
{
  ...
  size_t olen = 0;
  const psa_status_t status = psa_cipher_encrypt(key_id, PSA_ALG_ECB_NO_PADDING, in, 16, out, 16, &olen);
...
static void factory_init(void)
{
  ...
  const uint8_t colors[12] = {
    (uint8_t)(body >> 16),  (uint8_t)(body >> 8),  (uint8_t)(body),         (uint8_t)(button >> 16),
    (uint8_t)(button >> 8), (uint8_t)(button),     (uint8_t)(accent >> 16), (uint8_t)(accent >> 8),
    (uint8_t)(accent),      (uint8_t)(grip >> 16), (uint8_t)(grip >> 8),    (uint8_t)(grip),
  };
  char serial[15];
  ...
  ESP_LOGI(TAG, "factory serial=%s pid=0x%04x fw=%u.%u.%u", serial, NS2_PRO_PID, ver[0], ver[1], ver[2]);
}
...
static ns2_adv_mode_t adv_mode_for(ns2_identity_t identity, const uint8_t **out_mac)
{
  ...
  *out_mac = NULL;
  ns2_adv_mode_t mode = ns2_adv_choose_mode(ns2_session_paired(), s_ses.pairing_mode, &s_adv_win, esp_timer_get_time());
  /* 对账开关只钉窗口内的已配对形态（串口 adv）：连接窗口默认回连 0x00，
     * 对账时可强发唤醒 0x81 比较主机反应。 */
  if (s_window_form != NS2_WINDOW_FORM_AUTO && (mode == NS2_ADV_RECONNECT || mode == NS2_ADV_WAKE)) {
  ...
  const uint8_t *recorded_ptr = ble_creds_host_mac(identity, recorded) ? recorded : NULL;
...
static void adv_start_identity(size_t index, ns2_identity_t identity, const uint8_t payload[NS2_ADV_PAYLOAD_LEN])
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (mode == NS2_ADV_OFF) {
      if (ble_controller_adv_running(ids[i])) {
        ...
        ESP_LOGI(TAG, "silent: identity %u advertising stopped", (unsigned)ids[i]);
      }
      continue;
    }
    ns2_adv_payload(adv, NS2_PRO_PID, mode, mac);
    adv_start_identity(i, ids[i], adv);
    ESP_LOGI(TAG, "advertising: identity %u %s (%u creds)", (unsigned)ids[i], adv_mode_name(mode),
...
void ns2_session_connect(void)
{
  ...
  ESP_LOGI(TAG, "connect: advertising for %llds", (long long)(NS2_ADV_CONNECT_WINDOW_US / 1000000LL));
  apply_advertising();
}
...
void ns2_session_wake_request(void)
{
  ...
  ESP_LOGI(TAG, "wake: wake advertising for %llds", (long long)(NS2_ADV_WAKE_WINDOW_US / 1000000LL));
  apply_advertising();
}
...
void ns2_session_on_sync(const uint8_t own_mac[6])
{
  ...
  ESP_LOGI(TAG, "host synced, own MAC %02x:%02x:%02x:%02x:%02x:%02x", own_mac[0], own_mac[1], own_mac[2], own_mac[3],
           own_mac[4], own_mac[5]);
  ble_synced_set(true);
...
{
  ESP_LOGI(TAG, "%s session normal (conn=%u, fmt=0x%02x)", ns2_identity_name(ses->identity), ses->conn_handle,
           ses->report_format);
}
...
static void promote_if_host_registered(session_slot_t *ses)
{
  ...
  if (!ns2_adv_host_registered(ses->addr_matched, ses->pair_handshake_done, ses->features_enabled)) {
    return;
  }
...
bool ns2_session_set_adv_addr_form(uint8_t form)
{
  ...
  ESP_LOGW(TAG, "adv addr form -> %s (own %02x:%02x:%02x:%02x:%02x:%02x)", ns2_adv_addr_form_name(form),
           s_ses.own_mac[0], s_ses.own_mac[1], s_ses.own_mac[2], s_ses.own_mac[3], s_ses.own_mac[4], s_ses.own_mac[5]);
...
void ns2_session_on_connect(uint16_t conn_handle, uint8_t identity)
{
  ...
  ESP_LOGI(TAG, "connected (conn=%u, identity=%s, peer %02x:%02x:%02x:%02x:%02x:%02x, %s)", conn_handle,
           ns2_identity_name(identity), peer[0], peer[1], peer[2], peer[3], peer[4], peer[5],
...
void ns2_session_on_disconnect(uint16_t conn_handle, uint8_t identity)
{
  ...
  if (!s_ses.pairing_mode) {
    ...
    ESP_LOGI(TAG, "disconnected (conn=%u, identity=%s): opening 30s reconnect window", conn_handle,
             ns2_identity_name(identity));
  }
  apply_advertising();
}
...
static void build_mem_map(mem_map_entry_t *map, uint8_t identity)
{
  ...
  map[6].len = 24;
  map[6].data = (const uint8_t[]){ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static size_t handle_flash_cmd(uint8_t identity, const uint8_t *req, size_t len, uint8_t subcmd, uint8_t *resp,
                               size_t cap)
{
  if (subcmd != 0x04) {
  ...
  for (size_t i = 0; i < MEM_MAP_LEN; i++) {
    ...
    memcpy(&resp[16] + (overlap_start - addr), &map[i].data[overlap_start - block_start], overlap_end - overlap_start);
  }
  if (!hit) {
...
static size_t handle_init_cmd(session_slot_t *ses, const uint8_t *req, size_t len, uint8_t subcmd, uint8_t *resp)
{
  switch (subcmd) {
  case 0x01:
...
static size_t handle_feature_cmd(session_slot_t *ses, const uint8_t *req, size_t len, uint8_t subcmd, uint8_t *resp)
{
...
static size_t handle_pairing_cmd(session_slot_t *ses, const uint8_t *req, size_t len, uint8_t subcmd, uint8_t *resp,
                                 size_t cap)
{
  switch (subcmd) {
  case 0x01:
  ...
  case 0x02:
    ...
    if (!ses->pair_ltk_ready || len < NS2_FRAME_HEADER_LEN + 17 || cap < NS2_FRAME_HEADER_LEN + 17) {
      return 0;
    }
    {
      uint8_t a2[16];
...
static void log_hex_block(const char *what, const uint8_t *data, size_t len, size_t max)
{
  ...
  for (size_t off = 0; off < n; off += 16) {
    ...
    ESP_LOGI(TAG, "%s+%03u %s", what, (unsigned)off, hex_prefix(&data[off], chunk, line, sizeof(line)));
  }
  if (len > n) {
...
void ns2_session_on_command(const uint8_t *data, size_t len, uint8_t transport, uint16_t conn_handle)
{
  ...
  case 0x01:
    ...
    resp_len = ns2_nfc_on_command(data, len, subcmd, frame, sizeof(resp) - ANSWER_PREFIX_LEN);
    break;
  case 0x07:
  ...
  case NS2_CMD_SPI_FLASH:
    resp_len = handle_flash_cmd(ses->identity, data, len, subcmd, frame, sizeof(resp) - ANSWER_PREFIX_LEN);
    break;
  case 0x03:
  ...
  case 0x11:
    ...
    if (subcmd == 0x03) {
      static const uint8_t sensor_block[28] = {
        ...
        0xe8, 0x9c, 0x42, 0x58, 0xa0, 0x0b, 0x42, 0x0a, 0xe8, 0x9c, 0x41, 0x58, 0xa0, 0x0b,
      };
  ...
  case NS2_CMD_PAIRING:
    resp_len = handle_pairing_cmd(ses, data, len, subcmd, frame, sizeof(resp) - ANSWER_PREFIX_LEN);
    break;
  default:
  ...
  if (cmd == 0x01) {
    ...
    if (ns2_nfc_pop_event(&ev)) {
      ...
      if (ev_len <= sizeof(resp) - ANSWER_PREFIX_LEN) {
        ...
        ESP_LOGI(TAG, "nfc drain push 0x%02x (%uB)", ev.subcmd, (unsigned)(ANSWER_PREFIX_LEN + ev_len));
      }
    }
  }
  if (!hot_path) {
    ESP_LOGI(TAG, "rsp 0x%02x/0x%02x (%uB) %s", cmd, subcmd, (unsigned)(ANSWER_PREFIX_LEN + resp_len),
...
void ns2_session_on_output(const uint8_t *data, size_t len, uint16_t conn_handle)
{
  ...
  if (now - s_last_log_us >= 1000000LL) {
    ESP_LOGI(TAG, "rumble x%u/s: L=%u R=%u (0x%02x/0x%02x)", (unsigned)s_since_log, (unsigned)event.left_on,
             (unsigned)event.right_on, event.raw[0], event.raw[16]);
    s_last_log_us = now;
    s_since_log = 0;
  }
...
void ns2_session_start_pairing_mode(void)
{
  ...
  ESP_LOGI(TAG, "pairing request accepted (dropped link=%u, timeout=%llds)", (unsigned)was_connected,
...
static uint8_t s_fwupd_post[3] = { CONFIG_DEFAULT_FW_VERSION_MAJOR, CONFIG_DEFAULT_FW_VERSION_MINOR,
...
static void fwupd_schedule_apply(void)
{
  ...
  ESP_LOGI(TAG, "fw upgrade apply armed: report version -> %u.%u.%u, reboot soon", s_fwupd_post[0], s_fwupd_post[1],
           s_fwupd_post[2]);
}
...
static void fwupd_send_answer(uint16_t conn_handle)
{
  ...
  ESP_LOGI(TAG, "fw frame #%u complete: cmd 0x%02x/0x%02x body=%uB buffered=%uB -> rsp %uB",
           (unsigned)s_fwupd.up.frames, cmd, subcmd, (unsigned)ns2_upgrade_frame_body(&s_fwupd.up),
           (unsigned)s_fwupd.up.frame_len, (unsigned)s_fwupd_ack_len);
}
...
void ns2_session_on_fw_upgrade(const uint8_t *data, size_t len, uint16_t conn_handle)
{
  ...
  char line[160];
  const int used = snprintf(line, sizeof(line), "fw rec #%u +%ums gap=%ums type=0x%02x idx=%u %uB total=%u",
                            (unsigned)s_fwupd.writes, (unsigned)((now - s_fwupd.start_us) / 1000),
                            (unsigned)(gap_us / 1000), data[0], data[1], (unsigned)len, (unsigned)s_fwupd.bytes);
  const bool verbose =
      s_fwupd.writes <= 16 || (s_fwupd.writes % 32) == 0 || (s_fwupd.writes > 1 && len != s_fwupd.prev_len);
  if (verbose && used > 0 && (size_t)used < sizeof(line)) {
    char preview[3 * 16 + 4];
    snprintf(&line[used], sizeof(line) - (size_t)used, " %s", hex_prefix(data, len, preview, sizeof(preview)));
  }
  ESP_LOGI(TAG, "%s", line);
  s_fwupd.prev_len = len;
...
static void fwupd_finish(void)
{
  ...
  ESP_LOGI(TAG, "fw upgrade idle -> done: %u frames, %u records, %u bytes, span %us, rec %u..%uB",
           (unsigned)s_fwupd.up.frames, (unsigned)s_fwupd.up.records, (unsigned)bytes, (unsigned)(span_us / 1000000),
           (unsigned)(s_fwupd.up.min_record == SIZE_MAX ? 0 : s_fwupd.up.min_record), (unsigned)s_fwupd.up.max_record);
  if (s_fwupd.up.truncated) {
    ESP_LOGW(TAG, "fw upgrade: frame over %uB buffer", (unsigned)NS2_UPGRADE_FRAME_CAP);
  }
  log_hex_block("fw frame", s_fwupd.up.sample, s_fwupd.up.sample_len, sizeof(s_fwupd.up.sample));
...
void ns2_session_tick(void)
{
  ...
  if (s_fwupd_apply_us != 0 && esp_timer_get_time() >= s_fwupd_apply_us) {
    ...
    app_config_flush();
    ESP_LOGI(TAG, "fw upgrade applied: report version -> %u.%u.%u, restarting soon", s_fwupd_post[0], s_fwupd_post[1],
             s_fwupd_post[2]);
  ...
  if (!s_ses.pairing_mode) {
    for (size_t i = 0; i < SESSION_MAX; i++) {
      ...
      const bool subscribed = ble_controller_input_notify_ready(ses->conn_handle, ses->report_format);
      if (!subscribed || !ns2_adv_dormant_link(subscribed, ses->features_enabled)) {
        ses->dormant_ticks = 0;
        continue;
      }
      if (++ses->dormant_ticks < NS2_DORMANT_TICKS || s_dormant_drops >= NS2_DORMANT_MAX_DROPS) {
        continue;
      }
      s_dormant_drops++;
      ses->dormant_ticks = 0;
      ESP_LOGW(TAG,
               "dormant link (conn=%u, features not enabled) -> drop + reconnect "
               "(attempt %u/%u)",
               ses->conn_handle, (unsigned)s_dormant_drops, (unsigned)NS2_DORMANT_MAX_DROPS);
  ...
  for (size_t i = 0; i < SESSION_MAX; i++) {
    ...
    if (n == index) {
      ...
      if (ble_controller_input_notify_ready(conn, report_id)) {
        slot->reports++;
        if (slot->reports == 1) {
          ESP_LOGI(TAG, "first input report -> %s (conn=%u, fmt=0x%02x)", ns2_identity_name(slot->identity), conn,
                   report_id);
        }
      }
...
static bool apply_colors(uint32_t body_rgb, uint32_t button_rgb, uint32_t accent_rgb, uint32_t grip_rgb);
...
static bool apply_colors(uint32_t body_rgb, uint32_t button_rgb, uint32_t accent_rgb, uint32_t grip_rgb)
{
  const bool changed = s_ses.body_color != body_rgb || s_ses.button_color != button_rgb ||
                       s_ses.accent_color != accent_rgb || s_ses.grip_color != grip_rgb;
  if (!changed) {
    return false;
  }
  ...
  ESP_LOGI(TAG,
           "controller colors -> body=%06lx btn=%06lx accent=%06lx grip=%06lx "
           "(paired=%u pairing=%u)",
           (unsigned long)body_rgb, (unsigned long)button_rgb, (unsigned long)accent_rgb, (unsigned long)grip_rgb,
