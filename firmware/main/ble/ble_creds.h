 ...
 * 每个身份一份独立的记录列表。设备当前只模拟 Pro Controller 2，但分槽结构
 * 保留——换型号时主机眼中是另一台设备（序列号、PID、广播地址都不同），配对
 * 信息必须分开保存，未配对的身份进发现广播重新配对。记录语义对齐
 * controller.md「蓝牙配对信息区结构」配对区（主机 MAC + 16B LTK），MAC/LTK 均按配对指令线格式
 * 原样存储（反向字节序形态），回连广播直接使用存储值。
 */

/** NVS 布局里每个身份的槽位数（序列化按此固定长度，不随保留策略变化）。 */
#define NS2_CREDS_MAX 4
/** 内存里每个身份保留的凭证条数：对齐 controller.md「蓝牙配对信息区结构」的两条存储模型
