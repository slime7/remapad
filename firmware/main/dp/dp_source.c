...
void dp_source_inject(uint32_t buttons_mask, uint32_t hold_ms)
{
  ...
  portEXIT_CRITICAL(&s_inject_mux);
  ESP_LOGI(TAG, "debug key inject: mask=0x%08lx hold=%lums", (unsigned long)buttons_mask, (unsigned long)hold_ms);
}
...
void dp_source_inject_stick(char side, uint16_t x, uint16_t y)
{
  ...
  portEXIT_CRITICAL(&s_inject_mux);
  ESP_LOGI(TAG, "debug stick inject: %c x=%u y=%u", side == 'r' ? 'r' : 'l', (unsigned)x, (unsigned)y);
}
