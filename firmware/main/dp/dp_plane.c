...
static void ble_send_report(size_t index, uint8_t report_id, const uint8_t *body, size_t len, void *user)
{
  (void)user;
  (void)len;
...
static bool ble_session_info(size_t index, uint8_t *identity, uint8_t *report_format, void *user)
{
  (void)user;
...
static void feedback_commit(uint8_t fields, const pad_feedback_t *event)
{
  /* 最近一次采样事件的时间戳：脉冲的超时自灭按它算（esp_timer 读寄存器，
     * 放临界区外读，先后差一拍不影响语义）。 */
  const int64_t haptic_now_us = (fields & PAD_FEEDBACK_FIELD_HAPTIC) != 0 ? esp_timer_get_time() : 0;
  portENTER_CRITICAL(&s_feedback_mux);
  const bool was_active = s_feedback.haptic_sample_valid && s_feedback.haptic_sample != 0;
...
static void feedback_listener(ns2_feedback_type_t type, const void *payload, void *user)
{
  ...
  case NS2_FEEDBACK_RUMBLE: {
    ...
    event.rumble_key_count[PAD_TRIGGER_L2] = (uint8_t)ns2_rumble_keys(rumble->raw, keys);
    for (size_t k = 0; k < PAD_RUMBLE_KEY_COUNT; k++) {
      event.rumble_keys[PAD_TRIGGER_L2][k] = keys[k];
    }
    event.rumble_key_count[PAD_TRIGGER_R2] = (uint8_t)ns2_rumble_keys(&rumble->raw[16], keys);
    for (size_t k = 0; k < PAD_RUMBLE_KEY_COUNT; k++) {
 ...
 * 要先拿编码字节做变化判定，再决定发不发。
 */
static size_t encode_feedback_report_ex(const pad_feedback_t *feedback, uint8_t *out, size_t cap, bool quiet)
{
  uint16_t vid = 0;
  uint16_t pid = 0;
  pad_conn_t conn = PAD_CONN_UNKNOWN;
  if (!usb_input_device_ids(&vid, &pid, &conn) && !input_source_device_ids(&vid, &pid, &conn)) {
    return 0;
  }
  if (quiet) {
...
{
  portENTER_CRITICAL(&s_feedback_mux);
  const bool active = s_feedback.haptic_sample_valid && s_feedback.haptic_sample != 0;
  ...
  return pad_haptic_pulse_step(sample, (uint32_t)((now_us - start) / 1000), remain_ms, tone_hz);
}
...
static void dp_task(void *param)
{
  ...
  ESP_LOGI(TAG, "data plane task running, tick=%ums (power save %ums), target=%s", (unsigned)DP_TICK_MS,
           (unsigned)DP_POWER_SAVE_TICK_MS, target_name());
  ...
  for (;;) {
    ...
    if (power_save != last_power_save) {
      ...
      ESP_LOGI(TAG, "cadence -> %ums (%s)", (unsigned)tick_ms, power_save ? "power save: ble stack off" : "normal");
    }
    ...
    const uint8_t haptic_env = held_haptic_envelope(esp_timer_get_time(), &haptic_remain_ms, &haptic_tone_hz);
    ...
    if (feedback_sent_valid && pad_feedback_segment_changed(&feedback_sent, haptic_env, haptic_tone_hz)) {
      portENTER_CRITICAL(&s_feedback_mux);
      s_feedback_pending = true;
      portEXIT_CRITICAL(&s_feedback_mux);
    }
    if (s_feedback_pending) {
      ...
      if (feedback.haptic_sample_valid && esp_timer_get_time() - s_haptic_last_us > DP_HAPTIC_HOLD_US) {
      ...
      size_t feedback_out_len = encode_feedback_report(&feedback, feedback_out, sizeof(feedback_out));
      const pad_layout_t *feedback_layout = feedback_out_len > 0 ? pad_feedback_last_layout() : NULL;
      ...
      const bool hd_active = feedback_layout != NULL && feedback_layout->out.hd.ops != 0;
      ...
      const bool usb_audio_engaged = usb_input_attached() && feedback_layout != NULL &&
                                     feedback_layout->out.audio_haptic && usb_input_audio_haptics();
      const bool bridge_audio_engaged = input_source_attached() && feedback_layout != NULL &&
                                        feedback_layout->out.audio_haptic && s_bridge_audio_haptics;
      ...
      if (haptic_env == PAD_HAPTIC_PULSE && !usb_audio_engaged && !bridge_audio_engaged) {
        folded = feedback;
        pad_feedback_fold_pulse_motors(&folded, PAD_HAPTIC_PULSE);
        writeback_fb = &folded;
        feedback_out_len = encode_feedback_report(writeback_fb, feedback_out, sizeof(feedback_out));
      }
      if (usb_audio_engaged) {
        ...
        /* 每个子帧的帧数 = rate × cycle_ms / 1000 / 3（整数帧，引擎
                 * 按它倒数切帧）。 */
        haptic.slice_frames =
            (uint16_t)((uint32_t)feedback_layout->out.hd.rate_hz * feedback_layout->out.hd.cycle_ms / 3000u);
      ...
      if (usb_input_attached() && feedback_out_len > 0) {
        ...
        if (usb_out_len > 0 &&
            (!usb_out_valid || usb_out_len != usb_out_sent_len || memcmp(usb_out_sent, usb_out, usb_out_len) != 0)) {
      ...
      if (input_source_attached() && feedback_out_len > 0) {
        ...
        if (bridge_audio_engaged) {
          bridge_out_len = encode_quiet_feedback(&feedback, bridge_out, sizeof(bridge_out));
        }
        if (bridge_out_len > 0 && (!bridge_out_valid || bridge_out_len != bridge_out_sent_len ||
                                   memcmp(bridge_out_sent, bridge_out, bridge_out_len) != 0)) {
          ESP_LOGD(TAG, "feedback(bridge) -> %u bytes (%s)%s", (unsigned)bridge_out_len,
                   feedback_layout != NULL ? pad_family_name(feedback_layout->family) : "-",
      ...
      if (!feedback_sent_valid || !pad_feedback_equal(&feedback_sent, &feedback)) {
        ...
        const size_t feedback_payload_len =
            pad_feedback_wire(&feedback, hd_active ? &hd_render : NULL, feedback_payload, sizeof(feedback_payload));
    ...
    if (!ble_controller_connected()) {
      ...
      if (s_feedback.rumble_on[PAD_TRIGGER_L2] || s_feedback.rumble_on[PAD_TRIGGER_R2] ||
        ...
        pad_feedback_apply(&s_feedback, PAD_FEEDBACK_FIELD_RUMBLE | PAD_FEEDBACK_FIELD_HAPTIC, &cleared);
        s_feedback_pending = true;
        stale = true;
      }
      portEXIT_CRITICAL(&s_feedback_mux);
    ...
    if (pad.buttons != last_buttons) {
      ...
      if (now_us - last_button_log_us >= DP_BUTTON_LOG_MIN_INTERVAL_US) {
        ESP_LOGI(TAG, "buttons 0x%08lx -> 0x%08lx", (unsigned long)last_buttons, (unsigned long)pad.buttons);
