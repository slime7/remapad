...
static int ui_selected_colorway(const app_config_t *config)
{
  for (int index = 0; index < 4; ++index) {
    if (config->body_color == s_colorways[index][0] && config->button_color == s_colorways[index][1] &&
        config->accent_color == s_colorways[index][2] && config->grip_color == s_colorways[index][3]) {
      return index;
    }
  }
  return -1;
...
void ui_service_fill_state(remapad_ui_state_t *state)
{
  ...
  snprintf(s_ui.heap_text, sizeof(s_ui.heap_text), "%u / %u KB", (unsigned)((heap_total - heap_free) / 1024U),
  ...
  snprintf(s_ui.battery_text, sizeof(s_ui.battery_text), "%u%% · %u.%02uV", (unsigned)battery_get_percentage(),
           (unsigned)(battery_get_voltage_mv() / 1000U), (unsigned)((battery_get_voltage_mv() % 1000U) / 10U));
...
void ui_service_handle_action(const char *name, int value)
{
  ...
  if (strcmp(name, "brightness-up") == 0) {
    ...
    if (value >= 0 && value < 4) {
      app_config_set_controller_colors(s_colorways[value][0], s_colorways[value][1], s_colorways[value][2],
                                       s_colorways[value][3]);
    ...
    snprintf(command, sizeof(command),
             ...
             "\"captureKey\":%s}}",
             config->ds_touchpad_plus_minus ? "false" : "true", config->ds_capture_key ? "true" : "false");
    (void)js_bridge_submit_command(command);
  } else if (strcmp(name, "ds-capture") == 0) {
    snprintf(command, sizeof(command),
             "{\"t\":\"setDsBehavior\",\"id\":0,\"config\":{\"touchpadPlusMinus\":%s,"
             "\"captureKey\":%s}}",
             config->ds_touchpad_plus_minus ? "true" : "false", config->ds_capture_key ? "false" : "true");
    (void)js_bridge_submit_command(command);
