/**
 * 手柄固件更新的记录流装配（ns2_upgrade.c）：主机（Switch 2）把一条 0x0d/0x04
 * 命令帧切成记录经 0x0018 推下来，装配错位会让伪装应答答错帧，帧长判断错会让
 * 整包推不完。期望值与字节取自 2026-09-15 的实机抓包（主机系统 23.0.0，
 * 协议记录见 docs/controller.md「Command 0x0D - 手柄固件更新推送」）。
 */
#include "host_test.h"
...
/** 实机抓包：首帧装配流的前 320 字节（8 字节帧头 + 帧体开头）。 */
static const uint8_t k_frame_head[320] = {
    ,
};

/** 抓包事实：首帧 4108 字节，帧头声明体长 0x1004，体首 4 字节是块大小 0x1000。 */
...
static unsigned feed_frame(ns2_upgrade_t *up, const uint8_t *data, size_t len)
{
    ...
    while (off < len) {
        ...
        if (ns2_upgrade_feed(up, record, chunk + NS2_UPGRADE_RECORD_HEADER_LEN) ==
            NS2_UPGRADE_FRAME) {
            frames++;
        }
        index++;
...
{
    return (size_t)data[0] | ((size_t)data[1] << 8) | ((size_t)data[2] << 16) |
           ((size_t)data[3] << 24);
}
...
static void first_frame_assembles_into_one_command_frame(void)
{
    ...
    CHECK_EQ(up.bytes, 42u * (RECORD_PAYLOAD + NS2_UPGRADE_RECORD_HEADER_LEN) -
                           (RECORD_PAYLOAD - 8u));
...
static void short_record_is_counted_but_not_assembled(void)
{
    ...
    CHECK_EQ(ns2_upgrade_feed(&up, short_record, sizeof(short_record)),
             NS2_UPGRADE_MALFORMED);
...
static void frame_start_record_restarts_assembly(void)
{
    ...
    const uint8_t first[12] = {0x01, 0x00, 0x08, 0x00, 0x0D, 0x91,
                               0x01, 0x04, 0x00, 0x08, 0x00, 0x00};
...
static void completed_frame_gives_way_to_the_next(void)
{
    ...
    const uint8_t next[12] = {0x02, 0x01, 0x08, 0x00, 0x55, 0x66,
...
static void frame_over_the_buffer_never_completes(void)
{
    ...
    const uint8_t oversized[12] = {0x01, 0x00, 0x08, 0x00, 0x0D, 0x91,
...
HOST_TEST_SUITE(suite_ns2_upgrade, "ns2_upgrade",
                {"主机推送的首帧按记录装配成一条 0x0d/0x04 命令帧",
                 first_frame_assembles_into_one_command_frame},
