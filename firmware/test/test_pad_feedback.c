...
static void dualsense_usb_encodes_rumble_and_led(void)
{
  ...
  const size_t len = pad_feedback_encode(PAD_CONN_USB, 0x054C, 0x0CE6, &feedback, out, sizeof(out));
  CHECK_EQ(len, 48);
...
static void dualsense_bt_encodes_framed_report(void)
{
  ...
  const size_t len = pad_feedback_encode(PAD_CONN_BT, 0x054C, 0x0DF2, &feedback, out, sizeof(out));
  CHECK_EQ(len, 78);
...
static void haptic_sample_never_drives_motors(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  CHECK_EQ(pad_feedback_encode(PAD_CONN_USB, 0x054C, 0x09CC, &feedback, out, sizeof(out)), 32);
  CHECK_EQ(out[4], 0x00);
  CHECK_EQ(out[5], 0x00);
...
static void dualsense_usb_row_marks_audio_haptic(void)
{
  pad_feedback_t feedback = feedback_default();
  uint8_t out[PAD_OUTPUT_MAX];
  const size_t len = pad_feedback_encode(PAD_CONN_USB, 0x054C, 0x0DF2, &feedback, out, sizeof(out));
  REQUIRE(len > 0);
...
static void quiet_writeback_hands_the_coils_back_to_audio_haptics(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  pad_feedback_bt_seq_reset();
  CHECK_EQ(pad_feedback_encode_quiet(PAD_CONN_BT, 0x054C, 0x0DF2, &feedback, out, sizeof(out)), 78);
  CHECK_EQ(out[0], 0x31);
  ...
  CHECK_EQ(pad_feedback_encode_quiet(PAD_CONN_USB, 0x054C, 0x0DF2, &feedback, out, sizeof(out)), 48);
  CHECK_EQ(out[0], 0x02);
  CHECK_EQ(out[1], 0xA1);
  CHECK_EQ(out[2], 0x90);
  CHECK_EQ(out[3], 0x00);
...
static void ns2_pad_relays_lra_payload_verbatim(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  const size_t len = pad_feedback_encode(PAD_CONN_USB, 0x057E, 0x2069, &feedback, out, sizeof(out));
  CHECK_EQ(len, 42);
  CHECK_EQ(out[0], 0x02);
  CHECK_EQ(out[1], 0x7F);
...
static void ns1_rumble_encodes_host_waveform(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  const size_t len = pad_feedback_encode(PAD_CONN_USB, 0x057E, 0x2009, &feedback, out, sizeof(out));
  CHECK_EQ(len, 9);
...
static void ns1_rumble_follows_host_frequency(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  CHECK_EQ(pad_feedback_encode(PAD_CONN_USB, 0x057E, 0x2009, &feedback, out, sizeof(out)), 9);
...
static void xinput_writeback_has_no_report_id(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  const size_t len = pad_feedback_encode(PAD_CONN_USB, 0x046D, 0xC21F, &feedback, out, sizeof(out));
  CHECK_EQ(len, 8);
...
static void xbox_bt_writeback_uses_report_03(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  const size_t len = pad_feedback_encode(PAD_CONN_BT, 0x045E, 0x0B13, &feedback, out, sizeof(out));
  CHECK_EQ(len, 9);
  CHECK_EQ(out[0], 0x03);
  CHECK_EQ(out[1], 0x0F);
...
static void unknown_device_has_no_feedback_channel(void)
{
  ...
  uint8_t out[PAD_OUTPUT_MAX];
  CHECK_EQ(pad_feedback_encode(PAD_CONN_USB, 0x1234, 0x5678, &feedback, out, sizeof(out)), 0);
...
HOST_TEST_SUITE(suite_pad_feedback, "pad_feedback",
                { "采样音色的段边界按 tick 投递（段与段音高变化才算）", sample_segment_delivery_tracks_the_tick },
                { "HD 通路的振幅增益把安静档位抬进可感知区", hd_gain_lifts_quiet_amplitudes },
                { "手柄喇叭的输出路径显式路由到手柄喇叭", dualsense_rows_route_the_pad_speaker },
                { "音频触觉让位期间的写回把音圈交还给音频触觉", quiet_writeback_hands_the_coils_back_to_audio_haptics },
                ...
                { "DualSense 有线行声明音频触觉能力（usb_audio 接管的依据）", dualsense_usb_row_marks_audio_haptic },
                { "高频带强度随事件进持续帧，漏拷会把纹理震成全零", hf_band_strength_survives_into_held_frame },
                { "两带驱动频率随震动事件进持续帧", rumble_frequencies_survive_into_held_frame },
                { "震动写回不改写 DualSense 灯条，玩家号只上白灯", dualsense_rumble_leaves_lightbar_alone },
                ...
                { "登记的采样音色按各自节奏渲染而不是恒定强度", registered_samples_play_their_own_rhythm },
                { "未登记的采样回落缺省音色：一次短脉冲后静默", unregistered_samples_fall_back_to_one_pulse },
                { "协议清单的 0x01 低频蜂鸣按文档时长登记", lf_beep_sample_plays_documented_duration },
                { "采样音色按段给出幅度与剩余毫秒（蜂鸣器按段发声）", pulse_step_reports_amplitude_and_remaining },
                { "没有音频承载时查找手柄的强震段折进马达", pulse_segment_folds_into_motors_without_audio_haptics },
                { "定位呼叫先强震再两声上行短鸣（近似 Joy-Con 提示音）", locate_call_chirps_two_rising_tones },
                ...
                { "采样提示音不驱动马达（蜂鸣器与丢弃负责发声）", haptic_sample_never_drives_motors },
                { "NS1 的震动按主机波形写入（每侧两带各有频率与振幅）", ns1_rumble_encodes_host_waveform },
                { "NS1 的震动跟着主机下发的频带走", ns1_rumble_follows_host_frequency },
                { "NS1 的震动在协议没给频率时按缺省频率补足", ns1_rumble_fills_missing_frequency },
                ...
                { "等价反馈帧按写回语义判定（原始参数包不算变化）", equal_frames_follow_writeback_semantics },
                { "HD 映射按布局规则把主机波形重整成时序子帧", hd_render_maps_host_waveform_per_layout },
                { "采样音色的强震段铺音圈、发声段铺扬声器", hd_render_spreads_sample_segments },
                { "没有声明 HD 通路的布局行不产子帧", hd_render_needs_a_declared_layout },
                { "反馈线格式带基础段与 HD 子帧段", feedback_wire_carries_legacy_and_hd },
                { "等价判定跟子帧量化值与采样段边界走", equal_follows_quantized_ops_and_env },
