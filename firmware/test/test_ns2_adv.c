...
static const uint8_t s_discovery[NS2_ADV_PAYLOAD_LEN] = {
  0x02, 0x01, 0x06, 0x1B, 0xFF, 0x53, 0x05, 0x01, 0x00, 0x03, 0x7E, 0x05, 0x69, 0x20, 0x00, 0x01,
...
static const uint8_t s_reconnect[NS2_ADV_PAYLOAD_LEN] = {
  0x02, 0x01, 0x06, 0x1B, 0xFF, 0x53, 0x05, 0x01, 0x00, 0x03, 0x7E, 0x05, 0x69, 0x20, 0x00, 0x01,
  0x00, 0x81, 0xEB, 0x3A, 0xEB, 0xF1, 0x48, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/** 唤醒广播：带主机地址、状态位 0x81。 */
static const uint8_t s_wake[NS2_ADV_PAYLOAD_LEN] = {
  0x02, 0x01, 0x06, 0x1B, 0xFF, 0x53, 0x05, 0x01, 0x00, 0x03, 0x7E, 0x05, 0x69, 0x20, 0x00, 0x01,
  0x81, 0x81, 0xEB, 0x3A, 0xEB, 0xF1, 0x48, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
...
static void mode_choice_follows_window(void)
{
  ...
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_WAKE_BURST_US - 1), NS2_ADV_WAKE);
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_WAKE_BURST_US), NS2_ADV_RECONNECT);
  CHECK_EQ(ns2_adv_choose_mode(false, false, &win, t0), NS2_ADV_DISCOVERY);
  CHECK_EQ(ns2_adv_choose_mode(false, false, &win, t0 + NS2_ADV_WAKE_BURST_US), NS2_ADV_DISCOVERY);
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_CONNECT_WINDOW_US), NS2_ADV_OFF);
  CHECK_EQ(ns2_adv_choose_mode(false, false, &win, t0 + NS2_ADV_CONNECT_WINDOW_US), NS2_ADV_OFF);
  ...
  /* 窗口到期即静默。 */
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_WAKE_WINDOW_US), NS2_ADV_OFF);
}
...
static void reconnect_signal_never_wakes(void)
{
  ...
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_WAKE_BURST_US - 1), NS2_ADV_RECONNECT);
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_CONNECT_WINDOW_US / 2), NS2_ADV_RECONNECT);
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_CONNECT_WINDOW_US - 1), NS2_ADV_RECONNECT);
  CHECK_EQ(ns2_adv_choose_mode(true, false, &win, t0 + NS2_ADV_CONNECT_WINDOW_US), NS2_ADV_OFF);
...
HOST_TEST_SUITE(suite_ns2_adv, "ns2_adv", { "发现广播与样本一致", discovery_matches_capture },
