...
HOST_TEST_SUITE(suite_haptic_synth, "haptic_synth", { "静置参数输出纯零", idle_params_produce_silence },
                { "收震后音圈平滑收音且时长有界（短震动不被截没）", coil_ringdown_is_bounded_after_host_stops },
                { "整段静默后的新震动从子帧 0 起播", new_burst_after_silence_restarts_the_timeline },
                { "没有声音时扬声器恒零，触觉两路有波形", speaker_channels_stay_silent_without_sound },
                { "发声段的小喇叭音色铺在频道 1/2", speaker_tone_rides_channels_1_2 },
                { "发声段包络起音渐入收音渐出（段边界硬切听成咔哒）", speaker_tone_fades_in_and_out },
                { "发声段满幅钉在承载刻度上（不靠削顶）", speaker_tone_caps_at_scale },
                { "左右触觉通道各跟各的子帧", sides_follow_their_own_keys },
                { "子帧按时间顺序轮播，静默子帧保住节奏", keys_play_in_order_over_the_timeline },
                { "声明 1 个子帧的持续震动连续（不补静默切片）", single_declared_key_stays_continuous },
                ...
                { "相位跨块与跨子帧连续，切子帧不跳变", phase_stays_continuous_across_fills_and_keys },
