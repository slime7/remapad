...
static void ns2_09_body_parses_buttons_sticks_and_battery(void)
{
  ...
  CHECK_EQ(state.buttons, (uint32_t)(PAD_BTN_CROSS | PAD_BTN_CIRCLE | PAD_BTN_DPAD_DOWN | PAD_BTN_MUTE | PAD_BTN_R4));
...
static void ns1_pro_report_maps_buttons_sticks_and_motion(void)
{
  ...
  CHECK_EQ(state.buttons,
           (uint32_t)(PAD_BTN_SQUARE | PAD_BTN_CIRCLE | PAD_BTN_R1 | PAD_BTN_OPT | PAD_BTN_HOME | PAD_BTN_DPAD_DOWN));
...
static void ns1_joycon_simple_report_maps_side_buttons_and_stick(void)
{
  ...
  CHECK_EQ(state.native_lang, PAD_LANG_NS1);
  CHECK_EQ(state.native_identity, PAD_IDENTITY_JOYCON_L);
  CHECK_EQ(state.buttons, (uint32_t)(PAD_BTN_DPAD_DOWN | PAD_BTN_DPAD_UP | PAD_BTN_TOUCHPAD | PAD_BTN_L1));
...
HOST_TEST_SUITE(suite_pad_ns, "pad_ns", { "NS2 报文体按 PID 命中布局行并带上透传语言", ns2_pad_rows_resolve_by_pid },
                { "NS2 的 0x09 报文体解析按键、打包摇杆与电量", ns2_09_body_parses_buttons_sticks_and_battery },
