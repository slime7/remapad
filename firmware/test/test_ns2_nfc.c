...
static size_t run_cmd(uint8_t subcmd, const uint8_t *body, size_t body_len, uint8_t *resp, size_t cap)
{
...
static void test_read_buffer_chunks(void)
{
  ...
  for (uint16_t off = 0; off < NS2_NFC_BUFFER_TOTAL; off += NS2_NFC_READ_CHUNK_MAX) {
    ...
    CHECK_EQ(chunk, total + NS2_NFC_READ_CHUNK_MAX <= NS2_NFC_BUFFER_TOTAL ? NS2_NFC_READ_CHUNK_MAX
                                                                           : NS2_NFC_BUFFER_TOTAL - total);
...
HOST_TEST_SUITE(suite_ns2_nfc, "ns2_nfc",
                ...
                { "带 d0 07 描述符的写卡首块剥掉描述符、数据从页 4 流式落位", test_write_descriptor_stream },
                { "写卡落盘失败不拦住主机流程", test_write_sink_failure_is_not_fatal },
                { "0x0C 状态查询返回固定原值", test_nfc_status_body },
                { "应答帧头的 Status/ACK 字节按子命令取值", test_response_ack_pairs_follow_captures },
                { "0x06 触发后状态走 0x14→0x15，0x05 体首字节与报告字节同源", test_read_status_progression_14_to_15 },
