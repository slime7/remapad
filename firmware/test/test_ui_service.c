...
static void link_actions_submit_commands_with_notice(void)
{
  ...
  CHECK(strcmp(host_test_js_bridge_command(2), "{\"t\":\"debugKey\",\"id\":0,\"key\":\"home\"}") == 0);
  ui_service_fill_state(&s_state);
...
static void usb_role_actions_submit_role_commands(void)
{
  ...
  CHECK(strcmp(host_test_js_bridge_command(0), "{\"t\":\"setUsbRole\",\"id\":0,\"role\":\"host\"}") == 0);
  ui_service_handle_action("usb-device", 0);
  CHECK(strcmp(host_test_js_bridge_command(1), "{\"t\":\"setUsbRole\",\"id\":0,\"role\":\"device\"}") == 0);
}
