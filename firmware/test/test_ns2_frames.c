...
static void response_body_and_capacity(void)
{
  ...
  CHECK_EQ(ns2_frame_response(frame, sizeof(frame), NS2_CMD_VERSION, NS2_FRAME_TRANSPORT_BLE, 0x02, body, sizeof(body)),
           sizeof(frame));
  ...
  CHECK_EQ(
      ns2_frame_response(small, sizeof(small) - 1u, NS2_CMD_VERSION, NS2_FRAME_TRANSPORT_BLE, 0x02, body, sizeof(body)),
      0);
  ...
  CHECK_EQ(
      ns2_frame_response(header_only, sizeof(header_only), NS2_CMD_SPI_FLASH, NS2_FRAME_TRANSPORT_USB, 0x00, NULL, 0),
...
{
  /* 配对指令里的固定公钥：LTK = A1 XOR B1，
     * 写错一位主机就配不上，因此逐字节固定在这里。 */
  static const uint8_t expected[NS2_PAIR_PUBKEY_LEN] = { 0x5C, 0xF6, 0xEE, 0x79, 0x2C, 0xDF, 0x05, 0xE1,
                                                         0xBA, 0x2B, 0x63, 0x25, 0xC4, 0x1A, 0x5F, 0x10 };
  CHECK_BYTES(ns2_pair_pubkey_b1, expected, sizeof(expected));
}

HOST_TEST_SUITE(suite_ns2_frames, "ns2_frames", { "响应帧头字段", response_header_fields },
                { "应答体拼接与容量边界", response_body_and_capacity }, { "0x10 版本应答体", version_body },
