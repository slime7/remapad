/**
 * OTA 会话协议（ota_proto.c）主机端用例：载荷布局、序号判定、4 KB 聚合边界、超时与应答字节；
 * 错一位表现为升级卡住、写坏镜像或 PC 以为成功而设备没换分区。
 */
#include "host_test.h"
 ...
 * 返回这一帧的结论。
 */
static ota_proto_result_t feed_image(ota_proto_t *proto, uint32_t image_size, uint16_t seq, size_t data_len,
                                     sink_t *sink)
{
  ...
  if (data_len > OTA_DATA_MAX_LEN || data_len > image_size) {
    host_test_expect(false, "data_len <= OTA_DATA_MAX_LEN && data_len <= image_size", __FILE__, __LINE__);
    return misuse;
  }
...
static void window_end_marker_acks_at_once(void)
{
  ...
  ota_proto_result_t result = ota_proto_data(&proto, payload, len, false, 1000, sink_flush, &sink);
  CHECK(!result.reply);
...
static void repeated_duplicates_answer_rate_limited(void)
{
  ...
  result = ota_proto_data(&proto, payload, later_len, false, OTA_DUPLICATE_REPLY_MIN_INTERVAL_US, sink_flush, &sink);
  CHECK(result.reply);
  CHECK_EQ(result.code, OTA_CODE_SEQ_ERROR);
  CHECK_EQ(result.next_seq, 1);
  CHECK_EQ(result.received, 200);
...
static void data_needs_an_open_session(void)
{
  ...
  ota_proto_result_t result = ota_proto_data(&proto, payload, len, false, 0, sink_flush, &sink);
  CHECK_EQ(result.code, OTA_CODE_BAD_HEADER);
  CHECK(result.reply);
...
static void idle_session_times_out(void)
{
  ...
  const ota_proto_result_t result = ota_proto_tick(&proto, 1000 + OTA_SESSION_TIMEOUT_US + 1);
  CHECK(result.reply);
  CHECK_EQ(result.state, OTA_STATE_FAILED);
...
static void begin_restamps_the_idle_timer(void)
{
  ...
  CHECK_EQ(ota_proto_tick(&proto, answered_us + OTA_SESSION_TIMEOUT_US + 1).code, OTA_CODE_TIMEOUT);
}
...
static void ack_payload_is_a_golden_byte_sequence(void)
{
  ...
  static const uint8_t expected[OTA_ACK_PAYLOAD_LEN] = { 0x01, 0x03, 0x34, 0x12, 0x04, 0x03, 0x02, 0x01 };
  CHECK_BYTES(payload, expected, sizeof(expected));

  /* BEGIN 的应答在末尾追加 16 字节运行版本，不足补 0。 */
  const size_t with_version = ota_proto_encode_ack(&result, "1.2.3", true, payload, sizeof(payload));
...
HOST_TEST_SUITE(suite_ota_proto, "ota_proto", { "BEGIN 校验 magic 与镜像尺寸边界", begin_validates_magic_and_size },
                ...
                { "整窗重发的序号错误应答按最小间隔限流", repeated_duplicates_answer_rate_limited },
