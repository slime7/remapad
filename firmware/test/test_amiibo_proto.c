...
static void test_upload_completes_and_stores(void)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        const size_t chunk = sizeof(image) - off > AMIIBO_DATA_MAX_LEN ? AMIIBO_DATA_MAX_LEN
                                                                      : sizeof(image) - off;
...
static void test_upload_572_with_signature(void)
{
    ...
    while (off < sizeof(image)) {
        const size_t chunk = sizeof(image) - off > AMIIBO_DATA_MAX_LEN ? AMIIBO_DATA_MAX_LEN
                                                                      : sizeof(image) - off;
        const size_t len = make_data(payload, (uint16_t)off, &image[off], chunk);
        REQUIRE(amiibo_upload_data(&up, payload, len, 1001 + (int64_t)off).code ==
                AMIIBO_CODE_OK);
        off += chunk;
    }
    ...
    while (sent < NS2_NFC_TAG_SIZE) {
        const size_t chunk = NS2_NFC_TAG_SIZE - sent > AMIIBO_DATA_MAX_LEN
                                 ? AMIIBO_DATA_MAX_LEN : NS2_NFC_TAG_SIZE - sent;
        const size_t len = make_data(payload, (uint16_t)sent, &image[sent], chunk);
        REQUIRE(amiibo_upload_data(&up, payload, len, 3001 + (int64_t)sent).code ==
                AMIIBO_CODE_OK);
        sent += chunk;
    }
    const amiibo_upload_result_t short_end =
        amiibo_upload_end(&up, 4000, store_capture, NULL);
...
static void test_begin_rejects_bad_name(void)
{
    ...
    /* 32 字节名字比上限多 1。 */
    const size_t long_len = make_begin(begin, "0123456789abcdefghij0123456789ab",
                                       NS2_NFC_TAG_SIZE);
...
static void test_missing_chunk_stores_nothing_and_resumes(void)
{
    ...
    const size_t third_len =
        make_data(payload, 2 * AMIIBO_DATA_MAX_LEN,
                  &image[2 * AMIIBO_DATA_MAX_LEN], sizeof(image) - 2 * AMIIBO_DATA_MAX_LEN);
...
static void test_idle_session_times_out(void)
{
    ...
    const amiibo_upload_result_t t =
        amiibo_upload_tick(&up, 1000 + AMIIBO_SESSION_TIMEOUT_US + 1);
    ...
    const amiibo_upload_result_t late = amiibo_upload_data(&up, payload, len, 1000 + 2 *
                                                           AMIIBO_SESSION_TIMEOUT_US);
...
HOST_TEST_SUITE(suite_amiibo_proto, "amiibo_proto",
                {"三帧数据按偏移拼出完整镜像后落库并回 DONE",
                 test_upload_completes_and_stores},
                {"尺寸不是 540 字节的上传在 BEGIN 就被拒绝", test_begin_rejects_wrong_size},
                {"572 字节（带厂商签名）同样收满落库，缺签名段算尺寸不符",
                 test_upload_572_with_signature},
                {"空名字与超长名字的 BEGIN 都是坏头", test_begin_rejects_bad_name},
                {"数据缺一块时 END 不落库，补上缺口后续传完成",
                 test_missing_chunk_stores_nothing_and_resumes},
