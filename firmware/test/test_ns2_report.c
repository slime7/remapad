...
static void button_bits_09(void)
{
  static const button_bit_t cases[] = {
    ...
    { NS2_BTN_Y, "Y", 0x02, 2 },
    { NS2_BTN_R, "R", 0x02, 4 },
    { NS2_BTN_ZR, "ZR", 0x02, 5 },
    { NS2_BTN_PLUS, "PLUS", 0x02, 6 },
    { NS2_BTN_RSTICK, "RSTICK", 0x02, 7 },
    { NS2_BTN_L, "L", 0x03, 4 },
    { NS2_BTN_ZL, "ZL", 0x03, 5 },
    { NS2_BTN_MINUS, "MINUS", 0x03, 6 },
    { NS2_BTN_LSTICK, "LSTICK", 0x03, 7 },
    { NS2_BTN_DPAD_UP, "UP", 0x03, 3 },
    { NS2_BTN_DPAD_LEFT, "LEFT", 0x03, 2 },
    { NS2_BTN_DPAD_RIGHT, "RIGHT", 0x03, 1 },
    { NS2_BTN_DPAD_DOWN, "DOWN", 0x03, 0 },
    { NS2_BTN_C, "C", 0x04, 4 },
    { NS2_BTN_GL, "GL", 0x04, 3 },
    { NS2_BTN_GR, "GR", 0x04, 2 },
    { NS2_BTN_CAPTURE, "CAPTURE", 0x04, 1 },
  ...
  for (size_t index = 0; index < sizeof(cases) / sizeof(cases[0]); index++) {
    ...
    for (uint8_t offset = 0x02; offset <= 0x04; offset++) {
      const uint8_t want = offset == cases[index].offset ? (uint8_t)(1u << cases[index].bit) : 0x00;
...
static void button_bits_05(void)
{
  static const button_bit_t cases[] = {
    ...
    { NS2_BTN_A, "A", 0x04, 3 },
    { NS2_BTN_R, "R", 0x04, 6 },
    { NS2_BTN_ZR, "ZR", 0x04, 7 },
    { NS2_BTN_MINUS, "MINUS", 0x05, 0 },
    { NS2_BTN_PLUS, "PLUS", 0x05, 1 },
    { NS2_BTN_RSTICK, "RSTICK", 0x05, 2 },
    { NS2_BTN_LSTICK, "LSTICK", 0x05, 3 },
    { NS2_BTN_HOME, "HOME", 0x05, 4 },
    { NS2_BTN_CAPTURE, "CAPTURE", 0x05, 5 },
    { NS2_BTN_C, "C", 0x05, 6 },
    { NS2_BTN_DPAD_DOWN, "DOWN", 0x06, 0 },
    { NS2_BTN_DPAD_UP, "UP", 0x06, 1 },
    { NS2_BTN_DPAD_RIGHT, "RIGHT", 0x06, 2 },
    { NS2_BTN_DPAD_LEFT, "LEFT", 0x06, 3 },
    { NS2_BTN_L, "L", 0x06, 6 },
    { NS2_BTN_ZL, "ZL", 0x06, 7 },
    { NS2_BTN_GR, "GR", 0x07, 0 },
...
static void motion_capture_mode_is_monotonic(void)
{
  ...
  const uint32_t a0 = (uint32_t)first[0x14] | ((uint32_t)first[0x15] << 8) | ((uint32_t)first[0x16] << 16);
  const uint32_t b0 = (uint32_t)first[0x32] | ((uint32_t)first[0x33] << 8) | ((uint32_t)first[0x34] << 16);
  const uint32_t a1 = (uint32_t)second[0x14] | ((uint32_t)second[0x15] << 8) | ((uint32_t)second[0x16] << 16);
  CHECK_EQ(a0, 50000u);
...
HOST_TEST_SUITE(suite_ns2_report, "ns2_report", { "静置状态 0x09 整包逐字节", idle_report_09 },
                { "0x09 按键位表", button_bits_09 }, { "0x05 按键位表", button_bits_05 },
                { "摇杆 12 位打包可逆与布局", stick_packing }, { "电源与充电状态字节", power_and_charge_bytes },
