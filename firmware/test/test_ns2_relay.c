/**
 * NS2 目标的运动字段与同代透传主机端用例：透传时按键、摇杆、电量与运动块必须原样到达主机，
 * 只有本机会话决定的状态字节被改写。
 */
#include "host_test.h"
...
static bool fake_session_info(size_t index, uint8_t *identity, uint8_t *report_format, void *user)
{
  (void)index;
  (void)user;
...
static void fake_send_report(size_t index, uint8_t report_id, const uint8_t *body, size_t len, void *user)
{
  (void)index;
  (void)user;
  s_sent_id = report_id;
...
static void motion_sensor_mode_fills_09_block(void)
{
  ...
  for (size_t i = NS2_09_MOTION_SAMPLES * NS2_09_MOTION_SAMPLE_LEN; i < NS2_INPUT_09_MOTION_LEN; i++) {
...
HOST_TEST_SUITE(suite_ns2_relay, "ns2_relay", { "运动数据填进 0x05 的 IMU 字段", motion_fills_report_05_imu_field },
                { "实验运动块按样本填 0x09 的运动区", motion_sensor_mode_fills_09_block },
                { "透传保留设备载荷并重写状态字节", relay_keeps_device_payload_and_rewrites_status },
                { "透传要求报告格式对上（0x07 载荷不投给 0x09 会话）", relay_requires_matching_format },
                { "目标优先透传，关闭后回到解析重编码", target_prefers_relay_until_turned_off },
                { "透传路径的耳机状态与编码路径同源", relay_reports_headset_state_from_same_source });
