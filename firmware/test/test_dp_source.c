/**
 * 数据面输入源合成（dp_source.c）主机端用例：逐条钉住多路输入的叠加规则与调试注入。
 * 源注册表是进程级静态状态，用例按文件内顺序执行，新增用例不要假设注册表为空。
 */
#include "host_test.h"
...
static void registration_limit(void)
{
  ...
  dp_source_sample(&state);
  /* 注入叠加在合成结果之上。 */
  CHECK_EQ(state.buttons, (uint32_t)(PAD_BTN_CIRCLE | PAD_BTN_CROSS | PAD_BTN_DPAD_UP | PAD_BTN_HOME));
}
...
HOST_TEST_SUITE(suite_dp_source, "dp_source", { "合成规则：主源拥有摇杆与设备字段，其余只叠按键", composition_rules },
