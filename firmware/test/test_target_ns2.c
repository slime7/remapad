...
static bool capture_session_info(size_t index, uint8_t *identity, uint8_t *report_format, void *user)
{
  (void)user;
  if (index != 0) {
    return false;
...
static void capture_send_report(size_t index, uint8_t report_id, const uint8_t *body, size_t len, void *user)
{
  (void)index;
  (void)user;
...
HOST_TEST_SUITE(suite_target_ns2, "target_ns2",
                { "面键按位置映射到 NS2 的 A/B/X/Y（私有用 PS 键名）", face_buttons_keep_position_semantics },
                { "肩键、方向键、选择类与系统键", shoulders_dpad_and_system_keys },
                { "四颗背键按侧折进 GL / GR", back_buttons_fold_into_gl_and_gr },
                { "SHARE 与触摸板同帧双置只出减号（串流一颗键双写）", share_and_touchpad_in_one_frame_only_minus },
                ...
                { "上报主机的电量跟输入设备自报值走，板载电池兜底", pad_battery_overrides_board_battery },
                ...
                { "LRA 参数包逐帧解出完整波形（含静止包与样例）", rumble_keys_decode_the_full_waveform },
                { "零幅度的使能保活包不算在震", zero_amplitude_enable_is_not_rumbling },
                { "查找手柄页的载波不算在震（频率位不再漏进振幅）", carrier_level_envelope_is_not_rumbling },
                { "触觉采样 ID 按命令帧解出", haptic_sample_parse_follows_the_frame },
                { "搜索页样本回放：只发采样流、震动段全程静置", search_page_capture_replays_to_samples_only },
                ...
                { "串口 headset 覆盖值钉住上报的耳机字节", headset_override_pins_the_reported_byte });
