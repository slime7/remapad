...
| **串口 CLI** | `cd pc ; uv run python uartctl.py -p COMx status` | 行命令控制台（免复位打开、`log` 只读日志、`link` 看连接间隔、上报计数与广播形态、`wake` 强制重连主机、`adv` 切常态广播形态、`version` 看镜像版本与升级状态、`rollback` 回滚待验证镜像） |
